...
> **Status:** done (closed 2026-07-17 — phases 1–2 shipped on `main`: `4a98cbe` (`dev` — key-gated `SocialSentimentAdapter` over the LunarCrush topic endpoint, `lunarcrush_api_key` SecretsStore key, Bearer-header-not-URL secret hygiene, honest-empty degrade on no-key/429/transport/non-JSON/shape-broken, provider registry `source="x"`) + `104b8ac` (`dev` — `sentiment(source="x")` as one registry entry via the `make_x_source(secrets_store)` factory so the no-key path carries an honest "no key configured" note; `create_mcp_components` gained optional `secrets_store`; no toolset change; apiref regenerated). Clean Mode 4 (in-session at user direction), no blockers/majors; all done-when criteria pass at assertion level. 2026-07-17 live probe ([ADR-0103 §Amendment](../../adrs/0103-social-x-sentiment-source.md)): the $0 LunarCrush tier has ZERO v4 API access (nine endpoint families, all 402 "Individual or higher") — degrade proven live on BTC + AERO; minimum spend Individual $90/mo. **User decision at close: spend PAUSED — stay Reddit-first (Plan 0111) until X coverage is proven needed.** The source ships dormant + honest; phase 3 (live smoke incl. the AERO small-cap-coverage question) stays outstanding behind the spend decision and does not gate this close.)
> **Created:** 2026-07-15
