...
> **Status:** done — closed 2026-07-12. Phase 1 shipped on `main` in `cb64203` (`dev`, one commit, no branch, migration-free, no new dep, no output-schema/apiref change): `_DustAwarePriceSource` wraps the historical-price source so a `dust_tokens` (`token_key`-form `chain:address`, lowercased) leg resolves to `$0` instead of raising `_MissingPrice`, contributing $0 to basis/realized/unrealized across every lookup site (a wrap, not a per-call-site special-case — a superior deviation from the plan's literal wording), with a sorted disclosing note on the still-complete position; config `defi_dust_tokens` threads `__main__` → `create_app` → the `/defi/pnl` route (`app.state`) + MCP tool registration → `run_wallet_pnl` → engine; `portfolio_summary` rides the empty default. Phase 2 (`human` live smoke) **PASS**: with `base:0xef0fd52e…` listed, `compute_wallet_pnl` on `0xae5b…9790` reconstructs **5/5** complete — the Wanderers position `incomplete=False` at ~$0 with the disclosing note, and the wallet total no longer flagged `partial` (recorded in [`../consolidated-smoke.md`](../consolidated-smoke.md), Run 2). Clean Mode 4 — **no blockers/majors**; two minors (route/MCP wiring rests on the job-level test + the live smoke rather than a route-level assertion; `portfolio.py` untouched, riding the plan-sanctioned empty default) + one nit (the dust note is emitted only on the complete path). ADR-0085 accepted at this close.
> **Created:** 2026-07-12
