...
> **Status:** done — closed 2026-06-11. Three phases on branch `plan-0053-short-selling-support` (`a39b196` contract → `96478eb` engine → `84715a6` live-signal), merged `--no-ff` as `607c1c6`. Clean Mode 4 — no blockers, no majors; one minor (backtester synced `desktop/renderer/types/sidecar/trade.ts`'s `Trade.kind` to `'long' | 'short'` — correct but ui-builder's domain) + one nit (`live_signal.py` imports the private `_ordering_key` from `adapter.py`). Every named spec read at the assertion level: same-bar flip pinned exit-first in **both directions and emission-order-independent**; single-direction/no-pyramiding invariants; costs-always-hurt mirrored bps with the long-twin sign check; falling-price short → rising equity hand-worked per bar; short-bearing full metric set value-asserted against a hand-checked fixture; engine e2e short run net of costs. `ENGINE_VERSION` 0.2.0 → 0.3.0 with the golden fixture moving by **exactly that one field** — the no-silent-long-regression pin (the `+1.0`-multiply / unrearranged `units * price` expressions keep the long path bit-identical in IEEE 754). New Calmar guard for short-driven `equity ≤ 0`. No migration (the chain stays free); the `SignalKind`-exhaustiveness open question closed as "no" by grep. 1422 tests + 7 known Windows skips green on the merged tree. ADR-0050 accepted at this close. **Unblocks Plan 0054.**
> **Created:** 2026-06-08
