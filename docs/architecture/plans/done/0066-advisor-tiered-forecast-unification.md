...
> **Status:** done (2026-07-09). Phases 1–3 on `main` (`339d951` dev → `012d5fe` dev → `b081270` ui-builder), no branch, migration-free. Clean Mode 4 — no blockers, no majors; one minor followup (the renderer `FEATURE_SET_LABELS` hash→name map mirrors `forecast/features.py` with no cross-language pin — all three ids verified matching at close, ADR-0057-frozen, unmapped→raw-hash graceful). Every done-when read at the assertion level: `test_leg_matches_forecast_core_with_store_wired` runs a real in-memory metric store and asserts the advisor leg == the forecast tool's core (identical tier/prob/skill + byte-identical `_as_forecast_result`), the stated-v1 unwired fallback, the untrainable-block `ValueError`, the artifact round-trip, and the basis tier scalars. Close gates green: 87 Python specs (recommend/forecast/fusion) + 17 renderer specs, `mypy --strict` + ruff clean. Phase 4 (human live smoke — re-run the 0063 divergence case on the real sidecar) was verified in the 2026-07-12 consolidated live smoke (see [`consolidated-smoke.md`](../../consolidated-smoke.md)). Was approved 2026-07-08 (user — "approve all plans, do not start work"); implemented 2026-07-09 (user "go").
> **Created:** 2026-07-08
