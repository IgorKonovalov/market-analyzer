...
- **Renderer-source selection contract.** The runtime decision between "load from the Vite dev server" and "load `dist/renderer/index.html`" is governed by the **`ELECTRON_RENDERER_URL`** environment variable: set → `loadURL(env)`, unset → `loadFile(dist/renderer/index.html)`. The `dev` script sets it via `cross-env`; production launches and Playwright `_electron.launch` invocations leave it unset. The earlier prose above describes this gate as "when `NODE_ENV=development`"; the env-var form is the implemented contract because (a) `NODE_ENV` has other semantics inside Vite and Node, and (b) the alternative heuristic `!app.isPackaged` conflates "developer mode" with "no dev server reachable" — Playwright launches Electron unpackaged but does not run Vite, and that conflation produces `chrome-error://chromewebdata/` in e2e. `app.isPackaged` is still used, but only for the CSP `'unsafe-inline'` relaxation in `script-src`.
