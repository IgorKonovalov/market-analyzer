...
4. **Narrate the returned fields** — translate the JSON into the analyst's read, adding nothing the snapshot doesn't support:
   ...
   - **Active chart patterns** — `active_patterns` (Plan 0052): classical formations still in play (head & shoulders / inverse, double top/bottom, ascending/descending/symmetrical triangle, rising/falling wedge), each a `ChartPatternHit` with `pattern`, `state` (`forming` = geometry complete but no breakout yet — provisional, it can vanish; `confirmed` = a close broke the neckline/trendline by the ATR-scaled margin), `direction`, the defining `pivots`/`lines`, a measured-move `target` (a geometry fact, not a price call), and a 0–1 `strength`. Report the state explicitly — "an H&S is *forming*" and "an H&S *confirmed* its neckline break" are different facts.
