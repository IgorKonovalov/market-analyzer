...
> **Status:** done — code phases 1-3, 5-6, 8 landed on `main` (no branch): ph1 `00ab25b` (converging-envelope anchors), ph2 `c7f488e` (confirmed-only `projection` LineSeg + wire role), ph3 `e310fbd` + `3da9346` (apex extend/clip + dotted breakout arrow), ph5 `d373d6c` (H&S skeleton + downward projection), ph6 `bd67b3e` (skeleton polyline + shaded humps), ph8 `a1ba607` (base role) + `b596349` (double base line + projection) + `ddafb87` (test tidy). Clean Mode 4 close 2026-07-11: no blockers/majors. Gates green — 43 detector/tool py tests, 54 `trendlines.test.ts` + 8 `CandlestickChart.trendlines` renderer tests, ruff + mypy --strict + gen-types:check all clean. Tests read at assertion level (exact envelope anchors, projection endpoints/direction per family, 5-point skeleton order, base-line extremes, apex pixels, dash arrays, hump polygon). One recorded deviation (not a defect): ph5's skeleton ships as a draw-layer multi-point `TrendlineSpec` (`_hit_trendlines`), NOT a `LineSeg` — `LineSeg` is a 2-point model, so `role="skeleton"` lives on `TrendlineSpec.role` only; the behavioral done-when (5-pivot skeleton reaches `chart.trendlines`) is fully met. **Phases 4/7/9 (human visual smokes: symmetrical triangle + rising wedge, H&S, double bottom) are the user's outstanding step — not code gates; deltas fold back as followups.** Pattern backlog (ascending/descending triangle, falling wedge, inverse H&S — all foundation-covered, no new code) deferred to followup phases per user's close-now decision. ADR-0078 accepted at this close.
> **Created:** 2026-07-11
