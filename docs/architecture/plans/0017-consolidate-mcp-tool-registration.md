...
> **Depends on:** [Plan 0014](0014-interactive-chart-and-agent-mode.md) — its phases 1–2 (committed `b5cc6d2` / `7845ecd`) already added the inline `get_pending_ui_events` tool + `ui-events://recent` resource this plan sweeps into the `register_*` pattern, so the extraction target now exists in `mcp_app.py`. This plan still sequences **after** Plan 0014's close ceremony: its `ui-builder` phase 3 + `human` phase 4 remain open, and the discipline is not to interleave two open plans (phase 3 touches only `desktop/`, so the conflict surface is nil — sequencing is tidiness, not a hard technical gate).
...
- **Dependency: sequence this plan after [Plan 0014](0014-interactive-chart-and-agent-mode.md)'s close.** Plan 0014 phases 1–2 (committed `b5cc6d2`/`7845ecd`) already added the inline `get_pending_ui_events` tool + resource and the `ui_event_buffer` param on `create_mcp_components`, so the extraction target now exists. Its phase 3 (`ui-builder`, renderer-only) + phase 4 (`human`) + the architect close remain; since phase 3 touches only `desktop/`, 0017 *could* run in parallel without conflict, but the default discipline is to let 0014 close first rather than interleave two open plans.
