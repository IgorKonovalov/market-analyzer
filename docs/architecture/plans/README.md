...
| 0030 | [0030-lazy-historical-loading](0030-lazy-historical-loading.md) | implementation complete — pending 0031 + e2e | Scroll-left to load older bars. New `useOhlcvHistory` buffer hook + visible-range trigger + scroll-anchored prepend. **Both renderer phases shipped & green (`4526882`, `04f0758`)** but manual test showed the feature is dead: the Yahoo adapter can't fetch past-ending windows (now-relative `range=`), so paging returns ~0 older bars. Renderer correct; **blocked on [Plan 0031](0031-yahoo-absolute-range-fetch.md)** for the live path. Close-blockers: (1) the **deterministic seeded-cache Playwright e2e** — DONE (`0cd7f7a`, real, not a stub); (2) 0031 **phase 2** (finding M1) — the live "paging stops cleanly with no error chip" smoke fails until `/ohlcv` stops 500-ing the empty older-window (`UnknownSymbolError`) case. Not closeable until 0031 phase 2 lands + the live smoke re-runs clean. |
| 0031 | [0031-yahoo-absolute-range-fetch](0031-yahoo-absolute-range-fetch.md) | in-progress | Switch the Yahoo OHLCV fetcher from now-relative `range=` to absolute `period1`/`period2` Unix timestamps so a window ending in the past is fetched verbatim (`_yahoo_fetch.py`/`yahoo.py`). Lifts the artificial 732-day span cap for uncapped timeframes; folds in the 2026-05-31 audit's error-envelope guard. Phase 1 landed (`ce8067f` + route e2e gate `f93e431`). **Phase 2 pending (post-review finding M1):** the typed `UpstreamDataError` family still escapes `/ohlcv` as a **500** (route catches only `ResilientHttpError`/`ValueError`); maps per-kind + decides the backward-paging empty-window case. **Unblocks 0030** but also now **gates 0030's live close** (empty older window → `UnknownSymbolError` → 500 → error chip, not `reachedStart`). No new ADR yet (ADR-0007 intact; option (ii) may add a note). 2 phases, `dev`. |
...
_The `data` "Yahoo error-envelope not guarded" item was absorbed into [Plan 0031](0031-yahoo-absolute-range-fetch.md) (same file `_yahoo_fetch.py`, same parse path) after the 2026-06-02 lazy-loading manual test put that fetch path on the critical line — no longer a loose follow-up._

## Recommended execution order

The committed roster (Tier 2 0009–0012, then 0013–0014) is fully closed, as are the capability-expansion gates 0018, 0019, 0025, and 0027, and the analysis batch's 0021. **Pickup-ready now, all `approved` and on largely disjoint files (user may re-prioritise): 0020, 0022, 0023, 0026, 0031.** Serialize where files overlap:

- **0023 edits `App.tsx`/`client.ts`** — its collisions (0013/0014) are landed, so it now inherits rather than conflicts.
- **0026** (approved 2026-06-02, pickup-ready) opens the advisor + forecasting track ([ADR-0029](../adrs/0029-advisory-recommendation-boundary.md)/[ADR-0030](../adrs/0030-forecasting-subsystem.md), both proposed); it depends on nothing not already shipped and runs largely parallel to the analysis batch. Scoped at approval to the full `data/timeframes.py` set (15m/1h/4h/1d/1w) now that Plan 0025 has landed. Mixed-owner: `backtester` (phase 1) → `dev` (phase 2) → `ui-builder` (phase 3); phase 2 touches the event bus — now relocated to the neutral top-level `events/` core (Plan 0028, landed), so import from `market_analyser.events`, not `api.events`. Downstream (forecasting plan, advisor plan + new `advisor` skill, UI) is undrafted.
- **0031** (approved, Yahoo absolute-range fetch) is a small `dev` data-layer fix: `_yahoo_fetch.py` + `yahoo.py` only. It **blocks 0030's completion** (0030's renderer is done but dead without it) and should be picked up promptly to unstick that work. It brushes the same `data/adapters/` surface as **0022** (provider wiring) — sequence on a quiet data-layer window, don't run them in parallel. The Plan 0028 adapter-Protocol refactor has **landed**, so a new Yahoo fetch path conforms to `OhlcvSource` (`data/sources.py`) rather than colliding with in-flight work. No collision with the analysis/UI batch.
- **0030** (`implementation complete — pending 0031`, lazy historical loading) is `ui-builder`-only and its renderer phases are **shipped and green** — but the feature is **dead until [Plan 0031](0031-yahoo-absolute-range-fetch.md) lands** (the Yahoo adapter can't fetch past-ending windows). No further `ui-builder` work is needed; once 0031 ships, re-run the manual scroll smoke and run the close ceremony. It still **collides with 0026's** viewer-panel phase and any future overlay UI on the chart, and brushes `OhlcvView.tsx`, so coordinate there if 0030's close uncovers follow-up renderer work.
...
- **Numbering** is sequential, zero-padded to four digits, independent for plans and ADRs. **Next free plan: 0032. Next free ADR: 0033** (see [`../adrs/`](../adrs/)). Architect runs `Glob docs/architecture/plans/*.md` and `…/adrs/*.md` before drafting — never trusting memory.
