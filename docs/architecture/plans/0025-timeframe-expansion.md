...
Empty at draft time. Architect populates from review findings at the close ceremony. (Likely candidates: a `ui-builder` timeframe-picker extension for the new TFs; a general N-hour resampler if a later plan needs `2h`/`8h`.)
