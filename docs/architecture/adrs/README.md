...
| [0067](0067-ichimoku-in-trend-classification.md) | Ichimoku Cloud participates in the composed trend classifier (conjunctive-veto: can turn an EMA-stack trend into `SIDEWAYS`, never manufacture one), reading the correctly-displaced `senkou_*[i-displacement]` cloud-under-price | proposed (accepts at Plan 0073's close) | refines 0023's composed-snapshot classification | 0073 |
| [0068](0068-technical-read-advisory-tier.md) | A second, lesser sanctioned conditions→decision crossing: the "technical read" — one curated regime indicator → a direction, with NO conviction and NO levels, as a distinct `TechnicalRead` artifact (honesty by structural omission, not corroboration); the fused `recommend` all-legs-agree invariant untouched | proposed (accepts at Plan 0074's close) | extends 0029's advisory boundary (a lower tier); consumes 0023's indicators | 0074 |
...
- **Numbering** is sequential, zero-padded to four digits, never reused. **Next free ADR number is 0074** (0073 — execution engine topology / control-plane–data-plane, proposed 2026-07-11, exploratory foundational posture, no paired plan — is the highest drafted; the roster above is the full inventory, and per-ADR drafting history lives in each file's `Date:` header + `git log`). Still `proposed` and awaiting their paired plan's close: 0037 (DeFi risk — Plan 0042), 0041 (Polymarket — Plan 0040), 0043/0044 (execution — Plan 0044), 0067 (Ichimoku trend classification — Plan 0073), 0068 (technical-read tier — Plan 0074), 0070/0071 (forecast pivot — Plan 0077). Three exploratory posture ADRs sit `proposed` with **no plan-paired close ceremony** — they accept only if/when the user commits an execution plan: 0025 (execution feasibility; would accept at Plan 0046's close), 0072 (bounded-autonomy carve-out + Polymarket venue; accepts if/when an arb-execution plan commits — gated on Plan 0079's evidence), and 0073 (execution engine topology; accepts if/when an autonomous-engine plan commits). (0059 accepted at Plan 0064's close 2026-07-08; 0064 accepted at Plan 0070's close 2026-07-09; 0060 accepted at Plan 0065's close 2026-07-09; 0061 accepted at Plan 0067's close 2026-07-09; 0062 accepted at Plan 0068's close 2026-07-09; 0063 accepted at Plan 0069's close 2026-07-10; 0065/0066 accepted at Plan 0072's close 2026-07-10.) The architect runs `Glob docs/architecture/adrs/*.md` before drafting to pick the next number, never trusting memory. ADR numbers and plan numbers are independent sequences.
