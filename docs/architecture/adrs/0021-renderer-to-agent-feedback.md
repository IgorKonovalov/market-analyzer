...
- **Notification-surfacing observation (Plan 0014 close, 2026-05-30).** The phase-4 human smoke confirmed the polling path (`get_pending_ui_events`) end-to-end. Whether Claude Code surfaces `notifications/resources/updated` to the model as an automatic nudge was **not** separately confirmed at close — the contract is the polling tool regardless, and this remains the open question this ADR's "Negative consequences" flags. A future plan that wants to lean on the push path should re-measure it against the then-current Claude Code build before doing so.
