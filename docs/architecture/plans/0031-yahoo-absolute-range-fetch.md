...
- **What:**
  ...
  - **Open design question — `UnknownSymbolError` on a backward-paging empty window (decide before coding; may want architect).** The adapter collapses two distinct cases into `UnknownSymbolError`: (a) a genuinely unknown/unlisted symbol, and (b) a *valid* symbol whose requested older window predates its listing / Yahoo horizon — the "end of history" case Plan 0030 needs to read as `reachedStart`. The renderer latches `reachedStart` on an empty **200 `[]`**, not on an error. Options: **(i)** route `UnknownSymbolError` → 404 and have `useOhlcvHistory.loadOlder` treat a 404 on an *older-chunk* fetch as `reachedStart` (renderer change, re-opens a `ui-builder` touch on 0030); or **(ii)** distinguish at the data layer so a valid-symbol empty older window returns `[]` (200) — i.e. the empty→`UnknownSymbolError` heuristic only fires when the symbol itself is unresolved, not for a legitimately-empty past window. Option (ii) is cleaner for 0030's "no error chip" claim but is a semantic change to the Plan 0013 heuristic and likely deserves a short ADR note. Pick one explicitly; do not leave both cases as a 500.
- **Files touched:** `src/market_analyser/api/routes/ohlcv.py`; `tests/api/test_ohlcv_route.py` (replace/augment the false-confidence 502 test with one that drives the **real** `DefaultMarketDataProvider`+`YahooAdapter` chain via an injected fetcher that raises/returns each case — reuse the wiring in `tests/api/test_ohlcv_route_historical.py`); possibly `src/market_analyser/data/adapters/yahoo.py` + an ADR note if option (ii) is chosen; possibly `desktop/renderer/hooks/useOhlcvHistory.ts` (+ its test) if option (i) is chosen.
- **Done when:**
