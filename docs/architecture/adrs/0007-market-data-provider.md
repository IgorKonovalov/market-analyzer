...
There is a tension to acknowledge: a separate decision committed us to **lazy vendoring** — only bring in upstream modules as phases need them. A literal "rewrite everything upfront" interpretation of a unified provider would force us to vendor everything now. We reconcile this by inverting the order: the *Protocol* is declared upfront with stubs for every planned method, and each method's implementation lands when its underlying source is vendored. The Protocol is the schedule; each phase fills in one stub.
...
A `DefaultMarketDataProvider` implementation dispatches to per-source *adapters* (`adapters/yahoo.py`, `adapters/tradingview_screener.py`, `adapters/coingecko.py`, etc.). Each adapter is a thin port over the vendored upstream service for that source. Adapters are package-internal to `src/market_analyser/data/`; downstream code never imports them. Vendored files themselves remain untouched per [ADR-0003](0003-vendoring-strategy.md) — adapters wrap, not edit.
...
- An indirection layer that does not exist upstream — pulling future fixes from the upstream project requires translating across the adapter, not direct copy. The drift-check script from ADR-0003 reduces but does not eliminate this cost.
...
### Neutral
- The `data/` package becomes the only directory in the repo where vendored upstream code lives (alongside `data/adapters/` and `data/provider.py`). Provenance stays auditable.
