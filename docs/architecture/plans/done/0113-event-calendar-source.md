...
> **Status:** done — closed 2026-07-21. Dev phases 1–3 shipped (`532a2ef` macro FOMC seed + FRED, `3a30684` Finnhub earnings, `abc221b` keyless listings self-diff + migration 0012), lockfile de-flake `42c1cdf`. One discriminated `event_calendar(category=macro|earnings|listings)` tool (`EXPECTED_FULL_TOOLSET` 57→58), conditions-only, wall-clock-sensitive. Mode 4 review clean — no blockers, no majors; all named test files opened and read against done-when (78 pass, 2 Windows POSIX skips); apiref regenerates zero-diff; secret hygiene (FRED key in query with path-only logging, Finnhub via header) pinned in tests. ADR-0107 accepted at this close. **Phase 4 (human live smoke) outstanding** — real-key + fresh-env inert verification is the one `human` phase and cannot be run by the architect. Token unlocks remain deferred (ADR-0107).
> **Created:** 2026-07-21
