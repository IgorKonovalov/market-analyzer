...
> **Status:** done — closed 2026-07-11. All five phases on `main`, no branch, migration-free: `dev` ×3 (ph1 `ichimoku()` pure trailing indicator `7e86b25`, ph2 cloud-aware `_classify_trend` conjunctive veto `98c92c1`, ph3 additive `ichimoku` `OverlaySpec` kind `11ca0da`) → `ui-builder` ×1 (ph4 displaced filled-cloud + lagging-Chikou render `fb79b9e`, ran after Plan 0072 ph8) → `human` ×1 (ph5 live smoke, user-confirmed OK). Clean Mode 4 — no blockers, no majors, no minors, no nits; every done-when read at the assertion level. Anti-lookahead is genuinely defended: `ichimoku` is in the parametrized `test_anti_lookahead_truncation_invariance` set, the cloud-under-price read is `senkou_*[i-displacement]` (`snapshot.py`) and `test_snapshot_ichimoku_anti_lookahead` pins it on prefixes; the veto matrix exercises all four branches (UP/DOWN × confirm/veto) plus the SIDEWAYS-base-cannot-manufacture and undefined-cloud-falls-back cases; the phase-4 lib test pins the displaced mapping (`i+disp` Senkou/cloud, `i−disp` Chikou, projection to `79+disp` past the last candle) and the crossover colour split; the phase-3 test pins exact-set-fields serialization, byte-unchanged `ema` wire, and price/label/role rejection. The `IchimokuPrimitive` is attached in `CandlestickChart.tsx` and fed by `useIchimokuSeries` (with `rightOffset = displacement` reservation so the projected cloud lands on-screen) — integrated, not orphaned. `docs/reference/mcp-tools.md` regenerated with the new kind. Benign improvement over the plan text: `ichimoku()` defines from `max(conversion, base, span_b) - 1` (general, correct for custom periods) rather than the literal `span_b - 1` — identical under classic defaults, documented in the docstring/test. [ADR-0067](../adrs/0067-ichimoku-in-trend-classification.md) accepted at close. Close gates: 80 analysis/events pytest + 84 renderer jest green. **Cleared for its dependents: Plan 0075 ph1 (`ichimoku()`) and Plan 0074's Ichimoku technical-read leg.**
> **Created:** 2026-07-09
