...
def test_migration_applies_and_chain_stays_linear() -> None:
    ...
    # The chain head advanced to 0012 (Plan 0113's listing snapshots); it must
    # still be a single linear head, each new revision must chain onto its
    # predecessor, and 0007 must still chain onto 0006.
    assert heads == ["0012_listing_snapshots"], f"expected a single head, got {heads}"
    assert (
        script.get_revision("0012_listing_snapshots").down_revision == "0011_defi_position_watches"
    ), "Plan 0113's listing-snapshots migration must chain onto 0011"
    assert script.get_revision("0011_defi_position_watches").down_revision == "0010_watch_note", (
        "Plan 0099's position-watches migration must chain onto 0010"
    )
    assert (
    ...
    assert (
        ...
        == "0008_advice_ledger"
    ), "Plan 0081's purge migration must chain onto 0008"
