...
SENTIMENT_DESCRIPTION = (
    ...
    "error. "
    "source='reddit': keyless, or keyed app-only OAuth when configured "
    "(reddit_client_id + reddit_client_secret) — an upvote-weighted keyword-lexicon score "
    "over one fixed multi-subreddit crowd group (r/CryptoCurrency+Bitcoin+wallstreetbets+"
    "stocks+investing) searched for the symbol (source tag 'reddit'); the payload adds a "
    "`label` (Strongly Bullish..Strongly Bearish) and `sample_size` (scored-post count); "
    "Reddit rate-limits hard and 403-walls keyless JSON from some networks, so an empty "
    "result (score 0.0, all-zero breakdown) may be a rate-limit or block rather than genuine "
