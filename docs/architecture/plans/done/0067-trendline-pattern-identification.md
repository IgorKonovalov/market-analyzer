...
> **Status:** done — closed 2026-07-09. Three `ui-builder` phases on `main` (`7af295f` colour-by-type + dedupe → `759ad1e` hover tooltip → `591dc3f` grouped legend), no branch, renderer-only, migration-free. Clean Mode 4 — no blockers, no majors, no minors; every done-when read at the assertion level, driving the real `CandlestickChart` for the component claims: `dedupeTrendlines` drops the dashed twin of a solid geometry (forming-only/confirmed-only intact, order preserved); `trendlineColor` gives each of the 9 pattern types a distinct hue + a stable neutral for unknown/absent (both light `:root` and dark `[data-theme]`/`prefers-color-scheme` token sets); `hitTest`/`hitTestTrendline` return the spec near a line and `null` past the 5px tolerance; the crosshair-over-a-line component test renders "Head & shoulders — forming"; the grouped legend lists one row per (pattern type, state) with instance count, uncheck removes exactly that group's lines (re-check restores), and row-hover drives `setHighlightedGroup`/clears on mouse-out. ADR-0049's role→colour is explicitly superseded (documented in ADR-0061 + code comments — not a silent reversal); no wire/sidecar change as promised. The one architectural cost — added interaction state on the flagged `CandlestickChart` god-component — is an ADR-0061-acknowledged, deferred-to-decomposition tradeoff (Plan 0072 phase 8), not a finding. Tooltip/legend share the display-name + state helpers (single source of truth). Close gates: 510/510 renderer jest green (64 across the four touched suites), `pnpm typecheck` + `pnpm lint` clean. **ADR-0061 accepted at close.** No diagrams referenced trendline colour. Manual live checks (colour-coded/de-cluttered lines on BTC-USD; hover; legend toggle/highlight) are the user's outstanding step, not a code gate.
> **Created:** 2026-07-08
