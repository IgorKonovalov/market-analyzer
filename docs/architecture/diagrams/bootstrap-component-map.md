...
- **`defi_tx`** (migration `0006`, Plan 0035) — the immutable decoded-transaction cache for P&L reconstruction ([ADR-0036](../adrs/0036-defi-pnl-reconstruction.md)): one row per `(wallet, chain, hash)`, insert-or-ignore only (tx history never updates), full `DecodedTx` payload as JSON plus the `(mined_at, mined_at_block, in_block_index)` replay-ordering columns. A re-scan reads the cache and fetches only the gap past the newest cached timestamp.
- **`price_snapshots`** (migration `0007`, Plan 0035) — the P&L determinism mechanism ([ADR-0036](../adrs/0036-defi-pnl-reconstruction.md)): `(token, ts) → price`, first-write-wins (an existing snapshot is never overwritten), so a replay is byte-identical even if the upstream price API later revises its numbers. Garbage prices (zero/negative/non-finite) are rejected at the boundary and never snapshotted.
