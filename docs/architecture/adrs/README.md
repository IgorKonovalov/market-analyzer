...
| [0056](0056-self-warming-metric-store.md) | Self-warming metric store: background accrual rides the sidecar lifespan, on by default | proposed — accepts at Plan 0061 close | feeds 0051 (the store); mirrors 0055's lifespan pattern (separate duty/clock); makes 0054's v2 set evaluable; drives 0052/0053 sources | 0061 |
...
- **Numbering** is sequential, zero-padded to four digits, never reused. **Next free ADR number is 0057** (0056 — the self-warming metric store, proposed 2026-07-06, paired with Plan 0061 — is the highest drafted; the roster above is the full inventory, and per-ADR drafting history lives in each file's `Date:` header + `git log`). Still `proposed` and awaiting their paired plan's close: 0037 (DeFi risk — Plan 0042), 0041 (Polymarket — Plan 0040), 0043/0044 (execution — Plan 0044), 0056 (metric accrual — Plan 0061), plus the exploratory 0025 (accepts at Plan 0046's close). The architect runs `Glob docs/architecture/adrs/*.md` before drafting to pick the next number, never trusting memory. ADR numbers and plan numbers are independent sequences.
