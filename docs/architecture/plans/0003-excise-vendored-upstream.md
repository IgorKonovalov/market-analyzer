...
Rewrite the Yahoo OHLCV fetch as a small in-house function in our adapter package, delete the entire `src/market_analyser/data/vendored/` tree and `vendored.lock`, and scrub every reference to the companion `tradingview-mcp` repository from source code, docs, diagrams, skill `SKILL.md` / references, and `CLAUDE.md`. After this plan lands, the name survives only in [ADR-0003](../adrs/0003-vendoring-strategy.md) (superseded, historical), [ADR-0009](../adrs/0009-rewrite-data-layer-in-house.md) (the supersession), and this plan file. End-to-end OHLCV behaviour is preserved: `GET /ohlcv` still returns the same `Bar` shape and tests pass.
...
- **What:**
  ...
  - In [Plan 0001](0001-bootstrap.md) and [Plan 0002](0002-strategy-interface.md): plans are not append-only the way ADRs are, but Plan 0001 is `in-progress` and Plan 0002 is `draft` with phases that referenced vendored bring-in. Rewrite the affected sections to point at in-house implementation. Excise the "Vendoring manifest" subsection in Plan 0001 (or strike-through and link to ADR-0009 as the reason).
...
- **Files touched:** `docs/architecture/adrs/0003-vendoring-strategy.md`, `0004-strategy-interface.md`, `0006-persistence-layout.md`, `0007-market-data-provider.md`; `docs/architecture/plans/0001-bootstrap.md`, `0002-strategy-interface.md`; `docs/architecture/diagrams/bootstrap-component-map.md`.
