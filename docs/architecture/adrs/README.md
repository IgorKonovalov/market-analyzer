...
- **Numbering** is sequential, zero-padded to four digits, never reused. **Next free ADR number is 0065** (0064 — generated sidecar API reference, proposed 2026-07-09, paired with Plan 0070 — is the highest drafted; the roster above is the full inventory, and per-ADR drafting history lives in each file's `Date:` header + `git log`). Still `proposed` and awaiting their paired plan's close: 0037 (DeFi risk — Plan 0042), 0041 (Polymarket — Plan 0040), 0043/0044 (execution — Plan 0044), 0060 (glossary tooltips — Plan 0065), 0061 (trendline pattern colour/identity — Plan 0067), 0062 (user chart-style overrides — Plan 0068), 0063 (in-house i18n — Plan 0069), plus the exploratory 0025 (accepts at Plan 0046's close). (0059 accepted at Plan 0064's close 2026-07-08; 0064 accepted at Plan 0070's close 2026-07-09.) The architect runs `Glob docs/architecture/adrs/*.md` before drafting to pick the next number, never trusting memory. ADR numbers and plan numbers are independent sequences.
