...
flowchart LR
    ...
    subgraph Sidecar["Python sidecar (standalone process)"]
        ...
        Routes["Renderer routes (renderer bearer)<br/>/healthz · /ohlcv · /quote · /search · /annotations<br/>/backtests · /news · /scan_patterns · /agent_mode<br/>/ui_events · /defi/scan · /watches · /alerts<br/>/settings/* · /stop · /events (SSE)"]
        Bus["In-process event bus<br/>(per-subscriber asyncio queue)"]
        Tools["MCP tools (~35 + watch toolset)<br/>OHLCV + backfill · annotations · agent chart<br/>(show/update/highlight) · backtest + get_backtest<br/>+ walk-forward + compare · evaluate_signals<br/>· screener · news/sentiment/stocktwits · quote · search<br/>· analyze_symbol + multi-tf + volume scanners<br/>· scan_patterns · detect_levels · detect_chart_patterns<br/>· macro (snapshot/btc/fear-greed) · btc_cycle_snapshot<br/>· get_metric_series · derivatives_snapshot<br/>· forecast · recommend · scan_wallet<br/>· create/list/delete_watch + list_alerts · ui-events poll"]
        ...
        DeFi["defi/<br/>(wallet discovery, scan job,<br/>deep LP enrichment)"]
        Sched["alerts/ scheduler<br/>(lifespan asyncio loop: watches →<br/>edge-triggered alert.triggered, ADR-0055)"]
        Cache[("SQLite cache.sqlite<br/>bars, annotations, backtest_runs,<br/>metric_points, watches, alerts")]
        MCPRoute --> Tools
        Tools --> DL
        Tools --> Analysis
        Tools --> Engine
