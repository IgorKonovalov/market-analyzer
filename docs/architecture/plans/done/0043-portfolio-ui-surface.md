...
> **Status:** done (2026-07-21) — Pillar 4 (Portfolio) UI shipped. Two phases on `main`, no branch, migration-free, no new dependency: `e41b4db` `dev` ph1 (the `GET /portfolio` + `POST /portfolio/risk` REST surface — the renderer-bearer-gated twins of the `portfolio_summary` / `defi_risk` MCP tools, whose bodies were factored into a shared `_portfolio_summary_response` + `PortfolioSurfaceResponse` and `_defi_risk_response` reused verbatim so agent and viewer surfaces can never disagree, the track_record/backtests precedent) → `aaddd9f` `ui-builder` ph2 (the Portfolio view + DeFi risk panel: holdings/avg-cost/unrealized-P&L/exposure with per-leg as-of, an Aave + LP scenario-shock control → HF/liquidation-distance/IL, conditional probabilities rendered with their vol assumption inline, and no rebalance/exit/buy/sell affordance). **Clean Mode 4 — no blockers, majors, or minors.** Every named done-when read at the assertion level: the 10 `tests/api/test_portfolio_surface.py` specs (unblended per-leg as-of asserted `binance != manual`, scenario/conditional recompute, boundary 422s, 401 + MCP-bearer cross-tenant reject, 404 mount-gate) and the 6 `PortfolioView.test.tsx` specs (all six phase-2 claims at the DOM level; the ADR-0029 no-advice boundary is genuinely enforced — a word-boundary scan of every button plus the whole container text, not a comment) all pass; `gen-types:check` reports no drift; and the ph1 factoring reversed no behavior (the pre-existing `test_portfolio_tool.py` ×7 + `test_defi_risk_tools.py` ×11 stay green). The four referenced ADRs — [ADR-0042](../adrs/0042-cross-venue-portfolio-aggregation.md) (aggregation), [ADR-0037](../adrs/0037-defi-position-risk-forecast.md) (conditional-risk framing), [ADR-0017](../adrs/0017-live-ui-updates-via-sse.md) (SSE), [ADR-0029](../adrs/0029-advisory-recommendation-boundary.md) (facts-only boundary) — were all pre-accepted, so no ADR flip at close. The two ADR-0042 presentation hazards the plan named (stale legs must not look live; a conditional liquidation probability must not read as a prediction) are each defended by an asserted spec. Implemented directly in this working tree — no branch/worktree to merge or prune. **The portfolio chain (0041 core → 0042 risk → 0043 UI) is now fully closed.** Followups (scoped, not gaps): multi-asset scenario shocks; realized-P&L history view; `exposure_by_asset` net-underlying normalization (0041's carried minor — BTC vs BTCUSDT).
> **Created:** 2026-06-05
