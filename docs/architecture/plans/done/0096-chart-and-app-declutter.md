...
> **Status:** done — closed 2026-07-14 ([ADR-0089](../adrs/0089-chart-display-presets.md) accepted). All five `ui-builder` phases on `main`, no branch, renderer-only, migration-free: `3a77a30` ph1 (segmented timeframe — roving-tabindex group, `aria-pressed`, `<select>` gone) → `8b243b2` ph2 (inline `ChartLegend`, retires the LAYERS checklist, ADR-0077 provenance split preserved — remove only on user rows) → `e9019d1` ph3 (chart presets + persisted `ma.layerVisibility`; the load-bearing bit — visibility now survives a remount, proven end-to-end in `CandlestickChart.presets.test.tsx`; `candleType` untouched by presets) → `5ec2aa0` ph4 (collapsible `ChartSidePanel`, `ma.rightPanelCollapsed` defaults collapsed, left-rail gutter reserved) → `6420c4b`+`d2a13d6` ph5 (grouped `NavMenu`, backtest SSE auto-switch preserved). Phase-6 human smoke run live; its findings landed as three fix-forwards: `b001ff8` (market structure a toggleable layer, off by default), `d156251` (pane-crash fix — `addPane(true)`/`preserveEmptyPane` stops the lightweight-charts `_cleanupIfPaneIsEmpty` "Invalid pane index" double-free), `90aab84` (collapsible legend panel + accent Save-confirm). **Mode 4 — no blockers/majors**; two minors (both pre-disclosed, ticketed, non-blocking): (1) `LayersPanel.tsx` is orphaned dead code — component no longer rendered, survives only as the home of the still-used `ChartLayer` type + `AddOverlayForm`'s shared CSS; its resize apparatus + `ma.layersPanelWidth` key are dead (phase-4 done-when's "removed cleanly with its state" unmet) + a stale `CandlestickChart.tsx:449-451` comment → **followup: relocate `ChartLayer`, delete the component**; (2) the pane-crash regression guard (`panes.test.ts`) is registry-level only — the jest lightweight-charts mock doesn't model `_cleanupIfPaneIsEmpty`, so the live crash path isn't reproduced in tests → **followup: faithful mock**. Assertion-level audit confirmed genuine acceptance tests throughout (real unmount/remount persistence, provenance-scoped remove, `addPane(true)` pin). Renderer-only confirmed by diff across `3a77a30^..90aab84` — no `desktop/main/` or Python `src/` file touched; i18n en+ru parity holds. Six deferred legibility items captured as drafted [Plan 0105](0105-chart-legibility.md). Implemented directly in this working tree — no branch/worktree to merge or prune.
> **Created:** 2026-07-13
