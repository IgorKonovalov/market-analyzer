...
| 0031 | [0031-yahoo-absolute-range-fetch](0031-yahoo-absolute-range-fetch.md) | in-progress | Switch the Yahoo OHLCV fetcher from now-relative `range=` to absolute `period1`/`period2` Unix timestamps so a window ending in the past is fetched verbatim (`_yahoo_fetch.py`/`yahoo.py`). Lifts the artificial 732-day span cap for uncapped timeframes; folds in the 2026-05-31 audit's error-envelope guard. Phase 1 landed (`ce8067f` + route e2e gate `f93e431`). **Phase 2 pending (post-review finding M1):** the typed `UpstreamDataError` family still escapes `/ohlcv` as a **500** (route catches only `ResilientHttpError`/`ValueError`); maps per-kind + handles the backward-paging empty-window case. Open design question **resolved — option (ii)** ([ADR-0033](../adrs/0033-empty-ohlcv-response-by-recency.md), proposed; accepts at this plan's close): a valid symbol's empty *past-ending* window returns `[]` (200) → renderer `reachedStart`, while a now-ending empty window still raises `UnknownSymbolError` → 404 — discriminator is window recency via the provider's `_now`/`as_of` seam. **Unblocks 0030** but also now **gates 0030's live close**. 2 phases, `dev`. |
