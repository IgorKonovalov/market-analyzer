...
> **Status:** accepted (2026-07-13, at [Plan 0095](../plans/done/0095-lightweight-charts-v5-migration.md)'s close — the behavior-preserving swap + first real `addPane()` consumer landed green with no visual regression: v5.2.0 pinned exact, every series site on `addSeries`, both primitives on the v5 type names, OBV on its own real pane behind `lib/panes.ts`; 851 renderer jest + 58 test:main green, CSP unchanged)
> **Date:** 2026-07-13
