...
- **Head & shoulders / inverse** (pivot-matched humps): **filled.** The reference fills each hump between the pattern skeleton and the neckline, and the fill is what makes the three-hump shape legible. Adopted for these patterns (Plan 0083's H&S phase); the enclosed region and the skeleton polyline that bounds it are the added geometry.
- **Double top / bottom** (pivot-matched, but drawn as two horizontals): **no fill.** Its reference draws a horizontal neckline through the middle pivot plus a horizontal line through the two matching extremes — no shaded region, no skeleton zigzag. Adopted that way (Plan 0083's double phase).

Text labels on the pivots (LS / Head / RS / Target / neckline / support) are *not* adopted for any pattern — pedagogical clutter on a live chart; deferred to the hover tooltip as a followup.
