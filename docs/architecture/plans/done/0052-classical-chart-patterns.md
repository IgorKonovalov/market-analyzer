...
> **Status:** done — closed 2026-06-10. Four phases on branch `plan-0052-classical-chart-patterns` (`34ffe28` → `bf6ba58`, + ruff-format `ae1ea51`), merged `--no-ff` to `main` as `2c53853` (conflict-free — `main` had not moved off the merge-base). Clean Mode 4: no blockers, no majors; one minor (the phase-2 "full expected-toolset assertion" doesn't exist in the repo — the house per-tool registration test in `tests/api/test_mcp_tools.py` was used instead, matching `detect_levels`/`forecast`) + two nits (wire-stability specs live in `tests/api/test_show_tools.py`, not the plan's `tests/events/...`; `ChartPatternHit.pattern` is `str`, not a `Literal`). Every named spec read at the assertion level: per-pattern truncation-invariance pinned for all nine fixtures (exact hit-set equality at every truncation `k`), forming/confirmed transition computed from the module's own constants, never-breaking fixture never confirms, exactly-one-`chart.show` with anchor `(ts, price)` + dashed/forming / solid/confirmed style pins against a real bus, snapshot field-set pin + no-action guard extended in place, canvas-free pixel math + legend-toggle component tests on the real chart. 114 plan-touched Python + 426 desktop tests green. ADR-0048 + ADR-0049 accepted at this close.
> **Created:** 2026-06-08
