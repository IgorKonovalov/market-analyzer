...
flowchart LR
    ...
    subgraph Sidecar["Python sidecar (standalone process)"]
        ...
        Routes["Renderer routes (renderer bearer)<br/>/healthz · /ohlcv · /quote · /search · /annotations<br/>/backtests · /news · /scan_patterns · /scan_chart_patterns<br/>/track_record<br/>/ui_events · /defi/scan · /defi/pnl · /watches · /alerts<br/>/defi/position_watches · /defi/position_alerts<br/>/settings/* · /stop · /events (SSE)"]
        Bus["In-process event bus<br/>(per-subscriber asyncio queue)"]
        Tools["MCP tools (57 registered, pinned by the<br/>exhaustive toolset test; ADR-0104 granularity —<br/>modes are discriminator params)<br/>OHLCV + backfill · annotations + get_chart_drawings<br/>· agent chart (show/update/highlight) · backtest + get_backtest<br/>+ walk-forward + compare · evaluate_signals<br/>· screener · news · sentiment(source) · quote · search<br/>· analyze_symbol + multi-tf + technical_read<br/>· scan_watchlist(rank_by) · volume_read(kind) · price_structure(kind)<br/>· scan_patterns · detect_levels · detect_chart_patterns<br/>· detect_divergences · sector_rotation<br/>· macro (snapshot/btc/fear-greed) · btc_cycle_snapshot<br/>· get_metric_series · derivatives_snapshot<br/>· forecast(kind) · recommend · recommend_rebalance<br/>· get_track_record · prediction markets (odds/search/convergence)<br/>· scan_wallet · compute_wallet_pnl · defi_fundamentals<br/>· scan_pool_discrepancies<br/>· portfolio_summary · create/list/delete_watch + list_alerts<br/>· create/list/delete_position_watch + list_position_alerts<br/>· ui-events poll"]
        Analysis["analysis/<br/>(indicators, patterns, chart patterns,<br/>levels, snapshot, volume, cycles, multi-tf)"]
        DL["MarketDataProvider +<br/>per-capability sources + repositories<br/>(Yahoo · Binance klines/derivatives · Coinbase ·<br/>CoinMetrics · CoinGecko · F&G · Polymarket · …)"]
        ...
        PosMon["defi/ position monitor<br/>(lifespan loop, ADR-0093: dwell-qualified<br/>LP out-of-range edge → defi.position_alert,<br/>heartbeat on /healthz)"]
        Cache[("SQLite cache.sqlite<br/>bars, annotations, backtest_runs,<br/>metric_points, watches, alerts,<br/>defi_tx, price_snapshots,<br/>defi_position_watches/alerts")]
        MCPRoute --> Tools
        Tools --> DL
        Tools --> Analysis
        Tools --> Engine
