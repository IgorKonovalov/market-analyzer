...
> **Status:** done — closed 2026-07-13. Six code phases on `main`, no branch, migration-free, no new dep: `dev` ph1–4 (`2c98707` Fibonacci retracement/extension + `dominant_swing` auto-anchor; `43f8312` `market_structure` HH/HL/LH/LL + BOS/CHoCH; `de00a6a` classic pivot points (floor/Camarilla/Woodie) + anchored VWAP; `3ac91e8` snapshot `market_structure` + `nearest_fib_level` beside the UNTOUCHED `trend`, four MCP tools, `EXPECTED_FULL_TOOLSET` +4, apiref regenerated) → `ui-builder` ph5–6 (`f0511ad` fib/pivot/anchored-VWAP overlays — three new `OverlayKind`s with TS↔pydantic parity guard; `53f2dab` HH/HL/LH/LL labels + BOS/CHoCH markers + structural-trend badge beside the indicator trend, glossary + en/ru parity). Mode-4 verdict: clean, no blockers/majors. Verified at close: ADR-0084 core proven (`trend` byte-identical, `market_structure` a separate field — `test_trend_unchanged_and_structure_is_a_separate_read`); anti-lookahead truncation-invariance (structure per-`k`, fib future-bar, snapshot); degenerate zero-volume/zero-range → `None`; no action field (asserted); `apiref --check` exit 0; 81 Python + 101 desktop jest pass; desktop typecheck + lint green. Phase 7 human live-smoke deferred (user-run). ADR-0084 accepted at this close.
> **Created:** 2026-07-12
