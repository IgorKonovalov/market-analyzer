...
- **Numbering** is sequential, zero-padded to four digits, never reused. **Next free ADR number is 0056** (0055 — the in-sidecar watch scheduler, accepted at Plan 0060's close 2026-07-03 — is the highest drafted; the roster above is the full inventory, and per-ADR drafting history lives in each file's `Date:` header + `git log`). Still `proposed` and awaiting their paired plan's close: 0036/0037 (DeFi P&L / risk — Plans 0035/0042), 0041 (Polymarket — Plan 0040), 0042 (portfolio aggregation — Plan 0041), 0043/0044 (execution — Plan 0044), 0054 (forecast v2 — Plan 0059), plus the exploratory 0025 (accepts at Plan 0046's close). The architect runs `Glob docs/architecture/adrs/*.md` before drafting to pick the next number, never trusting memory. ADR numbers and plan numbers are independent sequences.
