...
> **Status:** done (2026-07-03) — all four phases on branch `plan-0060-watchlist-alerting-loop` (`7b2ba85` persistence + migration `0005_watches_alerts`; `8e2f50d` pure evaluators + edge reducer; `2da8f2b` scheduler + `alert.triggered v1` + four MCP tools; `c788a99`+`e40e5eb` renderer routes + Alerts view/toast), merged `--no-ff` to `main`. Clean Mode 4 — no blockers, no majors; every named done-when read at the assertion level: forming-bar no-lookahead per watch kind; edge-reducer full transition matrix + folded poll sequence (exactly one fire while true, re-fire after re-arm); `last_state` restart survival through a fresh engine over the same SQLite file; fake-clock integration with exactly-one envelope + interval gating + per-series fetch coalescing asserted; condition-only payload pinned structurally (field set + `extra="forbid"`, no recommendation-shaped fields — ADR-0029); evaluator/fetch failures contained per watch/series with the error surfaced in the `/healthz` heartbeat; renderer Zod-strict at the SSE boundary (smuggled `action` field dropped); toast + history prepend + disable round-trip + bearer-gated routes. The mid-plan `merge main into plan-0060` (`edb1fd9`) integrated 0039's renderer changes before phase 4 — the fan-out stagger worked as designed. Post-merge gates green: pytest 1681 passed / 7 Win-skips, renderer jest 404/404 (49 suites), `mypy --strict`. ADR-0055 accepted at this close. Bonus catches: unanchored `models/` gitignore bug fixed (was swallowing `persistence/models/`); dev-skill `--secret` flag doc drift noted (actual contract: `MARKET_ANALYSER_SECRET` env var).
> **Created:** 2026-06-09
