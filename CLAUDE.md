...
- **Where secrets live — one picture (stop re-deriving it).** Third-party credentials (data-source keys + RPC URLs) resolve through the sidecar's `SecretsStore.get(key)` ([`persistence/secrets.py`](src/market_analyser/persistence/secrets.py)): **(1)** env override `MARKET_ANALYSER_<KEY>` (uppercased, e.g. `MARKET_ANALYSER_BASE_RPC_URL`) if non-empty, else **(2)** the canonical `0600` `secrets.json` in the user-data dir ([ADR-0038](docs/architecture/adrs/0038-third-party-api-key-storage.md)), else **unset** (typed error, never a silent fallback). A repo-root **`.env`** (dev-only, gitignored) is **not a separate store** — the sidecar loads it into those same `MARKET_ANALYSER_*` env vars at startup (`override=False`; packaged builds load nothing), feeding layer 1. `.env`/env use `MARKET_ANALYSER_<UPPER>`, `secrets.json` uses the bare key; same key set: `zerion_api_key`, `graph_api_key`, `eth_rpc_url`, `base_rpc_url`, `binance_read_api_key`, `binance_read_api_secret`, `alchemy_prices_key`. **Separate homes** (not this store): the per-launch IPC bearer (`MARKET_ANALYSER_SECRET` → `sidecar.lock`), the MCP bearer (`mcp-secret.json`), and future trade-execution keys (OS keychain via `keyring`, [ADR-0044](docs/architecture/adrs/0044-trade-secret-store.md) — never in `secrets.json`/`.env`). **One-off scripts read via the real `SecretsStore`, never a bespoke bare env var.** Full walkthrough: README → Configuration.
