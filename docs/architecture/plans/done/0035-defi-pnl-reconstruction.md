...
> **Status:** done (2026-07-05) — phases 1–7 implemented by `dev` in one session directly on `main` (`03861c7` → `8686496`, one conventional commit per phase) plus the phase-8 smoke's live-payload hardening (`dfa7eb6`: NFT `nft_info` transfers skipped-not-fatal; zero-quantity position entries dropped as nothing-held — both encoded as offline fixture tests). Phase-8 `human` live smoke ran 2026-07-05 on the test wallet: cold run 52.9s (full multi-chain history paged + cached, no 429s), warm re-run 1.9s with zero Zerion page fetches; 1 of 5 positions fully reconstructed (realized +$4,015.16, basis $873.50), 4 honestly `incomplete` via the designed loud paths (unbooked swap + unclassified gauge events; one uncovered long-tail DefiLlama price); wallet totals honestly null per ADR-0036; Zerion FIFO cross-check $29,670.07, no gross-divergence warning; replay-derived figures byte-identical across runs. Mode 4 review clean (no blockers): all plan-named specs opened and verified substantive — incl. the exhaustive 41-tool MCP registration test this plan promised — and gates re-run at close (1795 offline tests, `mypy --strict`, `ruff`, `gen-types --check` all green). Migrations 0006/0007 form one linear chain after `0005_watches_alerts`, as required. ADR-0036 accepted at this close.
> **Created:** 2026-06-05
...
Recorded at close (2026-07-05), from the phase-8 smoke. Both are extensions of the documented v1 accounting scope, not defects — the smoke showed the loud-failure paths working as designed:

- **Extend the phase-5 hint vocabularies to Aerodrome gauge-interaction shapes.** Three of the smoke wallet's Aerodrome LPs came back `incomplete` partly because gauge deposit/withdraw/claim transactions fall outside the classifier's current hint vocabulary and surface as `unclassified`.
- **Book `swap` events in the replay engine.** v1 deliberately leaves `swap` unbooked (an honest `incomplete`, per the engine's "unbooked swap" note); booking swaps as basis-rotation would, together with the gauge vocabulary above, flip those three LPs to complete.
- **Long-tail historical-price fallback.** One position (Wanderers) is blocked by a missing DefiLlama price for one leg at 2025-01-13. ADR-0034's documented fallback (Alchemy keyed prices behind the same `HistoricalPriceSource` Protocol) is the designed remedy if this recurs; a single long-tail miss does not justify it yet.
