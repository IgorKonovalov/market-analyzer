...
> **Status:** done — all 4 `ui-builder` phases on `main` (`86baa2c` store+resolver → `886b1fb` chart wired to the resolver → `958ce77` Settings chart-style section → `bd2b702` candle series-type switch), implemented directly in this working tree (no branch/worktree). Clean Mode 4 (2026-07-09): no blockers/majors/minors — every done-when read at the assertion level. Verified: `chartStyle.test.ts` done-when a–e (no-override defaults + widths + `candles`; per-theme colour/width independence; candle-type + reset round-trip through `localStorage`; malformed/blocked-storage degradation; subscribe fires-then-unsubscribes); phase-2 spec drives a live EMA colour+width override applied via `applyOptions` with `createChart` called once (no remount) and a theme-flip honouring the override; phase-4 spec proves a candle-type change rebuilds (second `createChart` + old `.remove()`), the new main series is a line, and span+trendline primitives (`attachPrimitive ×2`) and markers re-attach; SettingsView specs cover per-active-theme colour/width writes, the "Editing <theme>" live region, reset clearing every override incl. candle-type, and up/down colour controls inert (`disabled`) in line/area mode. `readChartColors`/`overlaySeriesColor` fully removed — `resolveChartStyle` is the single read path (ADR-0062 option B). Close gates: 539/539 renderer jest (59 suites), `typecheck` clean, `eslint` 0 errors. ADR-0062 accepted at this close. Benign deviations: `ResolvedChartStyle` gained a `chrome` field (one place for every drawn colour); controls extracted to `ChartStyleControls.tsx`; `rebuildToken: candleType` threaded into the four chart hooks; width control is a `<select>` (in-range by construction). Satisfies the 0068 half of Plan 0072 phase 8's gate.
> **Created:** 2026-07-08
