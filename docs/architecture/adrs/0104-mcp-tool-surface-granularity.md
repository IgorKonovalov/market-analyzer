...
### Neutral
- The consolidation is a surface refactor, not a behavior change: the underlying `analysis/`, `forecast/`, and sentiment computations are untouched; only their MCP entrypoints are re-shaped. Determinism, anti-lookahead, and the conditions-vs-advice boundary (ADR-0029) are unaffected — `quality_rank` stays conditions-side (ADR-0096) as a `rank_by` mode.
