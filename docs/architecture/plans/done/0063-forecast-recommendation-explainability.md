...
> **Status:** done (2026-07-08). Three phases on `main`, no branch, migration-free: `2941363` (phase 1 — forecast OOS permutation-importance core + artifact) → `6ec2618` (phase 2 — `fuse()` structured gate trace + advice artifact) → `4d8f641` (phase 3 — the "why" rendered in both panels); plus one post-review fix-forward `31c6d9d` (extract the shared explanation-artifact writer to `mcp_tools/_artifacts.py`; correct the `_build_checks` docstring — the two Mode 4 items). Clean Mode 4 — no blockers, no majors; every named done-when read at the assertion level (the OOS-only permutation asserted by a spy over each fold's own test slice; the replay invariant `directional ⟺ every check passed` pinned across a directional, a one-blocker flat, and an all-legs-fail flat; determinism re-run pinned modulo the two documented run-provenance exceptions). Close-time gates green: 110 Python + 53 TS specs across the touched suites, `mypy --strict` + ruff clean. **Phase-4 live smoke verified through the running sidecar (2026-07-08):** `forecast BTC-USD 1d` returned per-horizon top drivers inline and wrote `runs/forecast/…/explanation.json` (v2-deep tier, 24 features ranked with mean+spread, plausible — h=21 leans on adx/atr_pct/dist_200w_ma/funding_rate/mvrv; honest no-edge at all three horizons today); a second identical call produced a byte-identical explanation modulo `started_at` + path; `recommend` on BTC-USD/rsi returned a directional long whose 13-gate trace replays to `long` from the trace alone, conviction `0.0666` correctly derived (P×edge) and honestly low, artifact numbers matching the wire; the Forecast "Why" panel confirmed rendering. Accepts [ADR-0058](../adrs/0058-forecast-recommendation-explainability.md). Followup opened: the advisor's forecast leg still uses the single-horizon v1 core (`_compute_forecast`), not the tiered multi-horizon path — unifying them is a future plan, not a 0063 defect.
> **Created:** 2026-07-06
