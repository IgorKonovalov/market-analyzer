...
| 0029 | [0029-candlestick-chart-decomposition](0029-candlestick-chart-decomposition.md) | approved | Decompose the 618-line `CandlestickChart.tsx` god component (audit): extract `useChartGestures` (drops 8 prop-mirror refs), move marker/overlay math to `lib/`, collapse the 4-spot overlay seam into one registry. Behavior-preserving; render-reflection harness is the gate. No new ADR (applies ADR-0008). 2 phases, all `ui-builder`. |
...
Minors surfaced by the codebase-health audit. The three majors became Plans [0028](0028-data-layer-boundary-hardening.md)/[0029](0029-candlestick-chart-decomposition.md); these are the small items. Same rule — pick up opportunistically; promote if one grows past ~half a day or couples other changes.
...
- **0028** (draft, audit cleanup) refactors `data/default_provider.py` + `data/adapters/` + relocates the event bus — it **collides with 0022** (adds a `get_macro_context` Protocol method + provider wiring) and touches the same provider/adapter surface as the data work in 0021. Sequence 0028 *before or after* 0022, not in parallel; land it on a quiet data-layer window. Phases 1-2 are pure `data/`; phase 3's event relocation touches `api/` import sites, so coordinate with any in-flight `api/` plan.
- **0029** (draft, audit cleanup) is `ui-builder`-only and edits `CandlestickChart.tsx` — disjoint from 0023's News view and 0028's Python work, but **collides with 0026's** viewer-panel phase and any future overlay UI. Behavior-preserving, so it can slot whenever the chart component is otherwise idle.
...
- **Numbering** is sequential, zero-padded to four digits, independent for plans and ADRs. **Next free plan: 0030. Next free ADR: 0033** (see [`../adrs/`](../adrs/)). Architect runs `Glob docs/architecture/plans/*.md` and `…/adrs/*.md` before drafting — never trusting memory.
