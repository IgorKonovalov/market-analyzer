...
> **Status:** done — closed 2026-07-10. All 6 impl phases on `main`, no branch (ph1 `46df6cf` → ph2 `174ca17` → ph3 `7d8b8c0` → ph4 `ad291f6` → architect amend `90995aa` → ph4b `70d9318` → ph5 `9834eea` → ph6 `150fec0`); ph7 human live smoke passed (user-confirmed 2026-07-10). Clean Mode 4 — no blockers, no majors, no minors. Every done-when read at the assertion level: the `t()`/plural resolver pins the real `Intl.PluralRules('ru')` one/few/many + en-US `#` counts; `parity.test.ts` checks both-direction key-set equality **and** ICU-arm-aware placeholder survival; `test_fusion.py` pins `directional ⟺ every gate passed` on the reason-code surface, 1:1 condition/signal-code↔prose order with raw-token params, **unchanged English prose**, and determinism on both code surfaces; `RecommendationsView.test.tsx` ph5 proves the views render from codes (PROSE_* sentinels never appear) with a no-code prose fallback. Phase-4b reference regen was correctly a no-op (apiref renders `Recommendation` one level deep; nested `RecommendationBasis.condition_codes`/`signal_codes` don't surface — ph4 already added the top-level `reason_codes` row). Accepted English residue documented in `ru.ts:15`. Close gates: **585/585** renderer jest · **127/127** advisor+forecast pytest · `mypy --strict` + ruff clean · `tsc` (5 projects) + eslint (incl. the `no-unkeyed-literals` i18n guard) clean. ADR-0063 accepted at close.
> **Created:** 2026-07-08
