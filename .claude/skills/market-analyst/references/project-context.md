...
- ✅ **Mode 3 (screener)** — `screener_query` (TradingView universe, Plan 0009) for universe screens; `analyze_symbol` per-symbol + `scan_watchlist(rank_by=…)` for watchlist screens — rank/filter a caller-supplied symbol list by squeeze tightness, gainers/losers, momentum, composite quality, or a volume breakout / surge (the six scanners unified into one tool by Plan 0109; see the tool's `rank_by` enum for the exact mode names). Single-symbol volume backing is `volume_read(kind="confirmation")`.
