...
- **0029** (in-progress, audit cleanup) is `ui-builder`-only and edits `CandlestickChart.tsx` — disjoint from 0023's News view and 0028's Python work, but **collides with 0026's** viewer-panel phase and any future overlay UI. Behavior-preserving, so it can slot whenever the chart component is otherwise idle.
- **0030** (draft, lazy historical loading) is `ui-builder`-only. Phase 2 edits `CandlestickChart.tsx`, so it **depends on 0029** — sequence it after 0029 lands. Phase 1 (the `useOhlcvHistory` buffer hook) touches no chart file and could start in parallel with 0029 if desired. Also brushes `OhlcvView.tsx`, so coordinate with 0026's viewer-panel work on that file.
