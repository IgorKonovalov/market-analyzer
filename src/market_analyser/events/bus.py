"""SSE event bus runtime — pub/sub, envelopes, per-subscriber queues (ADR-0017).
...
`EventBus.publish(type, payload)` which:
  ...
  1. Validates `type` against the registered vocabulary (`TYPE_REGISTRY` in
     `events/payloads.py`; `UnknownEventTypeError` otherwise).
...
The MCP tools are the publishers. The renderer's `useEventStream` hook (ui-builder)
is the consumer. Split out of `events/__init__.py` in Plan 0072 phase 2; the
payload schemas live in `events/payloads.py` and both are re-exported from
`events/__init__.py` so the public import surface is unchanged.
"""
...
from market_analyser.events.payloads import TYPE_REGISTRY, ChartUpdateDroppedPayloadV1
