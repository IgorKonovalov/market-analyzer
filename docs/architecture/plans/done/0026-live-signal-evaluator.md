...
> **Created:** 2026-05-30
> **Approved:** 2026-06-02
> **Closed:** 2026-06-05 — clean Mode 4 (no blockers, no majors; 3 doc/consistency nits, all fix-forward). All three phases on `main`: phase 1 `d65e841` (`backtester` — pure `backtest/live_signal.py` core + `SignalEvaluation`/`EvaluatedSignal` models), phase 2 `5f54510` + fix `d12f4f4` (`dev` — `evaluate_signals` MCP tool + `signal.evaluated v1` event in the neutral `events/` core; optional fields defaulted `None` for `exclude_none`/schema consistency), phase 3 `f042e47` (`ui-builder` — reactive `LiveSignalView` panel, mounted at `App.tsx` `view==='signals'`). Verified at close: 28 Python specs (`tests/backtest/test_live_signal.py` + `tests/api/test_evaluate_signals_tool.py`) + 20 renderer specs (`LiveSignalView.test.tsx` + the `events.test.ts` TS↔pydantic parity guard) all green; every done-when defended by a value-asserting test (the load-bearing `signals_to_trades`-drops-it-we-keep-it divergence pinned by an actual `== []` call); both 2026-06-05 amendments (now-read safety property over `extra="forbid"` rejection; renderer TS in phase 3) faithfully implemented; the flagged `events → backtest.types` dependency confirmed acyclic (no `backtest → events` back-edge). No branch (committed on `main`). Nits logged as follow-ups. No ADR accepted (0026 is a condition-reporter; ADR-0029/0030 accept at Plans 0038/0036).
...
- **Mode 4 close nits (2026-06-05, all fix-forward — `dev` opportunistic):**
  1. `api/mcp_tools/evaluate_signals.py` docstrings (module docstring ~L22-24; `register_evaluate_signals` docstring ~L145-147) still claim a stray `as_of`/`range_end` key "fails the strict input schema" / "is rejected". The 2026-06-05 amendment + `test_live_extra_key_is_ignored_now_read_unaffected` prove the key is **ignored** (harmless — the read is structurally a now-read), not rejected. Reword to match the amended reality. Behavior is correct; only the prose is stale.
  2. `desktop/renderer/types/events.test.ts` header comment (~L7) references the pre-Plan-0028 path `src/market_analyser/api/events/__init__.py`; it is now `src/market_analyser/events/__init__.py` (the test's actual import is already correct — comment-only).
  3. (Pre-existing, already a tracked 2026-05-31 audit item — *not new to this plan*.) `useEventStream.ts:231` casts the `signal.evaluated` payload raw (`as SignalEvaluatedPayloadV1`) with no Zod `safeParse`, consistent with every other event. The `events.test.ts` parity guard pins the shape at build time. Folds into the existing "SSE payloads cast unvalidated" desktop follow-up if it's ever promoted.
