...
> **Status:** done (2026-07-06; approved 2026-07-06) — implemented on `main`, no branch: `ad509c6` (dev — `MetricAccrualJob` + lifespan wiring + `/healthz` heartbeat + `AppConfig` flags; one documented inversion: `create_app`'s own parameter defaults DISABLED because the job ticks at startup and an engine-wired test app must never reach the network — the product on-by-default lives in `AppConfig`, passed through by `__main__`) → `85f4c9d` (dev — `ForecastProvenance.fallback_reason` + the wired-but-starved v1 fallback with the derived bound `_v2_min_usable_rows(n_splits) = 2·n_splits`; the vacuous `n_scored=0` shape pinned non-producible; v2 wire dump pinned byte-stable) → `70865ec` (ui-builder — Zod `.nullish()` + the footer sentence through the real dispatcher, plus the no-regression footer spec), plus two phase-4 live-smoke fix-forwards on the OI seed, both upstream behaviors probed live and encoded in the offline fake transport: `41db964` (Binance `openInterestHist` now 400s any paged startTime ≥ 720h old — seed window shrunk to (30·24−1)h) and `6746334` (a past-end cursor now echoes the newest row instead of answering empty — read as end-of-data past the probe-anchored latest; the stuck-cursor guard holds mid-window). Clean Mode 4 same day — no blockers, no majors; every named done-when read at the assertion level. Close-time live verification through the running sidecar: all five series warm (F&G 3074 / funding 7475 / MVRV 5303 full backfills; OI 500 hourly buckets ≈ the ~21d real retention; first dominance bucket 2026-07-06T16:00Z) and a live `forecast BTC-USD 1d` returned genuinely-scored v1 blocks (`n_scored` 1025/1021/1005) with the insufficient-history `fallback_reason` stated — the 2026-07-06 vacuous-no-edge production state is dead. Two phase-4 residuals are time-gated and self-evidencing, not gates: the second dominance/OI bucket after the next hour boundary, and the off-switch reboot check. ADR-0056 accepted at this close.
> **Created:** 2026-07-06
