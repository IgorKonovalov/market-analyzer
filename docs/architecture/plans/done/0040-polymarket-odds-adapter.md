...
> **Status:** done (closed 2026-07-11) — both `dev` phases on `main`, no branch, migration-free, no new dependency: ph1 `4a3ecdb` (`PredictionMarketSource` Protocol + `PolymarketOddsAdapter` over the auth-free Gamma API + boundary-validated `PredictionMarket`/`MarketOutcome`, prob ∈ [0,1]), ph2 `0f4a4ab` (`search_prediction_markets` + `prediction_market_odds` tools, always-registered keyless, `EXPECTED_FULL_TOOLSET` 45→47). Mode 4 verdict: clean — no blockers/majors/minors. Every done-when read at the assertion level (52 tests: 27 adapter + 25 tool/toolset); ruff + mypy --strict + apiref --check clean; full non-network suite green (2192). Deviations from the plan's *illustrative* shape, self-flagged by dev, no new ADR: volume/liquidity ride the market (upstream reports them per-market, not per-outcome), and Gamma's `outcomePrices` (verified live to equal the CLOB midpoint) yields the CLOB-derived probability in one keyless call — the diagram's separate CLOB leg is a live-book followup. ADR-0041 accepted at this close.
> **Created:** 2026-06-05
> **Owner skill(s):** dev
