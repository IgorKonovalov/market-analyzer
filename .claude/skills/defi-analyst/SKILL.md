...
If the user asks for something that *would* benefit from new code (e.g. "set up a daily snapshot job for my positions"), stop and route to `architect` for a plan — don't start writing modules under `src/market_analyser/defi/`.
...
- **Write production code under `src/market_analyser/defi/`.** That goes through `architect` → `dev`. You can write throwaway analysis scripts in `runs/defi/<...>/scripts/` if it speeds up the analysis — but those are not the project's library code.
