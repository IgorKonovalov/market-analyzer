...
| [0065](0065-neutral-ui-event-buffer-core.md) | Move the renderer→agent poll buffer (`UIEventBuffer`/`UIEventEnvelope`) out of `api/` into a neutral `ui_events/` core so domain loops don't import the web layer | proposed (accepts at Plan 0072's close) | applies 0032's neutral-core precedent (done for the SSE bus) to 0021's poll buffer; unblocks 0055's scheduler from importing `api` | 0072 |
| [0066](0066-sse-short-lived-ticket.md) | Short-lived single-use SSE ticket in the `/events` URL instead of the durable renderer bearer (EventSource can't send a header) | proposed (accepts at Plan 0072's close) | refines 0002/0011's bearer transport for the one endpoint 0017 (SSE) can't header-authenticate | 0072 |
...
- **Numbering** is sequential, zero-padded to four digits, never reused. **Next free ADR number is 0069** (0068 — technical-read advisory tier, proposed 2026-07-09, paired with Plan 0074 — is the highest drafted; the roster above is the full inventory, and per-ADR drafting history lives in each file's `Date:` header + `git log`). Still `proposed` and awaiting their paired plan's close: 0037 (DeFi risk — Plan 0042), 0041 (Polymarket — Plan 0040), 0043/0044 (execution — Plan 0044), 0065/0066 (codebase remediation — Plan 0072), 0067 (Ichimoku trend classification — Plan 0073), 0068 (technical-read tier — Plan 0074), plus the exploratory 0025 (accepts at Plan 0046's close). (0059 accepted at Plan 0064's close 2026-07-08; 0064 accepted at Plan 0070's close 2026-07-09; 0060 accepted at Plan 0065's close 2026-07-09; 0061 accepted at Plan 0067's close 2026-07-09; 0062 accepted at Plan 0068's close 2026-07-09; 0063 accepted at Plan 0069's close 2026-07-10.) The architect runs `Glob docs/architecture/adrs/*.md` before drafting to pick the next number, never trusting memory. ADR numbers and plan numbers are independent sequences.
