...
> **Status:** done (2026-06-05) — all five `dev` phases shipped on `main` (`326335c` ph1 F1-fix + `pool_address` → `ec4263d` ph2 model fields + `LpPositionDetailSource` Protocol → `612fb71` ph3 Aerodrome one-hop RPC adapter → `32d3998` ph4 Uni-v3 two-hop + `tokenId` resolution → `fea9cfd` ph5 enrichment wired into the scan flow + tool/route). Clean Mode 4: no blockers, no majors; every named spec value-asserting (F1 dedup `WETH==0.18`, exact tick/fee decode, `in_range` both branches, typed RPC errors, one-hop vs two-hop call patterns, spacing). 1036 offline pytest + `mypy --strict` + `ruff` + `gen-types --check` (no drift) green. Enrichment is **fail-safe** (a wrong ABI / outage leaves a position at discovery depth, never fails the scan). **Two live verifications deferred** to user-run steps (need real secrets + network) and tracked as index follow-ups: the **Aerodrome Slipstream live smoke** against `0xae5b…9790` (confirms the flagged one-hop `positions()`-on-pool ABI unknown) and the **F3 Uni-v3 live-confirm** (no in-scope wallet holds one — ships fixture-tested). **No new ADR** (implements ADR-0034's already-accepted RPC+Graph stance).
> **Created:** 2026-06-05
> **Owner skill(s):** dev
