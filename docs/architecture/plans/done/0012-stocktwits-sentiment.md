...
- **(post-close correction — fixed) Implementation failed two pre-push gates; close review missed it.** Running the full `.husky/pre-push` gate (`ruff check` + `ruff format --check` + `mypy --strict src tests`) after the close surfaced defects the per-phase work and the close review both let through: (a) **phase-1 ruff escape** — `tests/data/test_stocktwits_adapter.py` carried an unused `k3` binding (F841) and a 102-char line (E501) plus format drift; this rode in on the mislabeled `91ac637` commit, which bypassed the per-phase ruff gate a normal `dev` phase-1 session runs; (b) **phase-2 `mypy --strict` regression** — extending the `get_sentiment` Protocol signature with `source` left **13 test-double `get_sentiment` methods** with the stale signature, so `mypy --strict src tests` reported 15 structural-conformance errors. Phase 2's done-when ("mypy strict passes including the signature extension") was therefore **not** actually met. **Root cause:** neither the implementer nor the close review ran the full project gate — the close's mypy check was *file-scoped* to the four touched source files, which is why it read clean. Fix: synced the 13 fakes to `source: str = "rss-vader"` (matching the existing `market: str`-against-`Literal["crypto"]` precedent) + cleared the ruff escapes; full gate now green (157 files mypy-clean, 460 offline tests pass). **Process lesson:** a done-when that says "mypy strict passes" is verified by running the project gate (`mypy --strict src tests`), never a file-scoped subset.
