...
| 0030 | [0030-lazy-historical-loading](0030-lazy-historical-loading.md) | implementation complete — pending 0031 | Scroll-left to load older bars. New `useOhlcvHistory` buffer hook + visible-range trigger + scroll-anchored prepend. **Both renderer phases shipped & green (`4526882`, `04f0758`)** but manual test showed the feature is dead: the Yahoo adapter can't fetch past-ending windows (now-relative `range=`), so paging returns ~0 older bars. Renderer code is correct; **blocked on [Plan 0031](0031-yahoo-absolute-range-fetch.md)**. Close ceremony deferred until 0031 lands + the manual scroll smoke passes. |
| 0031 | [0031-yahoo-absolute-range-fetch](0031-yahoo-absolute-range-fetch.md) | approved | Switch the Yahoo OHLCV fetcher from now-relative `range=` to absolute `period1`/`period2` Unix timestamps so a window ending in the past is fetched verbatim (`_yahoo_fetch.py`/`yahoo.py`). Lifts the artificial 732-day span cap for uncapped timeframes; folds in the 2026-05-31 audit's error-envelope guard. **Unblocks 0030.** No new ADR (adapter-internal; ADR-0007 contract intact). 1 phase, `dev`. |
