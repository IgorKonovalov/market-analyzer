...
- **Discriminated-result envelope for disjoint-shape clusters (added 2026-07-15, from Plan 0109 phase 2).** FastMCP wraps any tool whose *return annotation* is a `Union`/generic type in a `{"result": …}` object (`mcp/server/fastmcp/utilities/func_metadata.py` → `wrap_output=True`); only a single `BaseModel` return stays unwrapped. So a cluster whose modes return **different top-level models** — `forecast` (`MultiHorizonForecastResult` / `VolatilityForecast` / `RegimeForecast`), `price_structure` (four distinct `*Response` models), `volume_read` (two) — cannot keep each mode's *exact* top-level shape. Plan 0109 gives each such tool a purpose-built envelope `{<discriminator>, result: <mode payload>}` — itself a single object, so it is **not** generically wrapped — nesting each mode's existing model one level under `result`. The `result` payload is byte-identical to the retired tool's output and the SSE events are unchanged (they publish the raw domain models, not the tool return); only the agent-facing tool-return envelope gains a discriminator + one level of nesting. Clusters immune to this: `scan_watchlist`, whose six tools already shared one `{matches, skipped, scanned_at}` wrapper (the mode-union lives inside the `matches` **field**, so the return stays a single object — shipped this way in phase 1), and `sentiment`, whose two tools already return `dict` (already `result`-wrapped today), so consolidation is a zero-shape-change.
...
- **Discriminated-result shape (decided 2026-07-15, mid-implementation).** Phase 1 (`scan_watchlist`) shipped as `{rank_by, matches, skipped, scanned_at}` — the shared-wrapper case, no envelope needed. Phase 2 surfaced the FastMCP union-wrap constraint above; the accepted resolution is the `{discriminator, result}` envelope for the disjoint-shape clusters (full consolidation retained, 62 → 50), **not** dropping those merges — dropping would leave the largest routing-confusion cluster (`price_structure`, four near-identical single-symbol reads) unconsolidated, defeating the rule's purpose. Concrete envelope contract per cluster lives in Plan 0109.
