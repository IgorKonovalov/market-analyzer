...
Close review fired 2026-05-26 (in-session, after the four phases landed: commits `a89b7ca`, `f9bf40c`, `edd9ac2`, `beccf62`, `87ca34f`). **No blockers.** Offline Python suite 511 passed / 4 known Windows skips / 5 network-deselected; renderer Jest 131 passed; `mypy --strict` clean (164 files); ruff clean. Every phase carries a single in-vocabulary owner tag. The anti-lookahead `as_of` seam is preserved — the backfill path (`get_ohlcv_with_status`, the coordinator) has **no** `as_of` parameter, so a backfill is structurally impossible under backtest mode. No ADR was silently reversed; ADR-0015's "docstrings are agent UX" intent is the whole point of the plan and is met (the `get_ohlcv` description no longer says "from the local cache").

**Fixed at close (2026-05-26):**
- **M1 (was major) — toast render path untested.** Phase 4's done-when named a failure→toast assertion; only `useBackfillState`'s error *state* was unit-tested, not the `OhlcvView`→`Toast` render. Added `desktop/renderer/views/OhlcvView.test.tsx` (commit `465cf9d`, `ui-builder`): asserts the error toast renders with reason+message, the header spinner shows on `started` then clears on `failed`, a non-matching `(symbol, timeframe)` is ignored, and dismiss hides the toast.
- **m2 (was minor) — `get_ohlcv` skipped boundary validation + stuck-spinner on non-typed errors.** `_get_ohlcv_response` now validates symbol/timeframe/range at the MCP boundary like `backfill_ohlcv` does; `BackfillCoordinator._run_backfill` now publishes `ohlcv.backfill_failed` for ANY fetch exception (non-typed → `upstream_unavailable`) before re-raising, so the renderer spinner always clears (commit `9cd9952`, `dev`).
- **Golden-path smoke (parallel session) — `get_ohlcv` shape change broke the non-pytest smoke driver.** Phase 2's `list[Bar]` → `GetOhlcvResponse` change broke `tests/smoke/golden_path.py` step 2 ("expected a list, got dict"). Fixed in commit `1706e74` (`dev`, parallel session): a pure `unwrap_ohlcv_bars` helper (+ unit test) that returns bars on a clean read, maps an upstream `partial_reason` to UPSTREAM-DOWN, and FAILs on a malformed shape / unexpected `backfill_async_pending`. Note for future reviews: the golden-path smoke driver is **not** pytest-collected, so an offline `pytest` run does not catch this class of break — eyeball the smoke driver whenever an agent-facing MCP tool's response shape changes.

**Carried (minor / nit — pick up opportunistically):**
- **m3 (minor, `ui-builder`) — the phase-4 spinner Playwright case (`live-chart.spec.ts`) runs against live Yahoo and branches on the tool's `started` flag**, so on a warm cache it skips the spinner appear/clear assertions (only "bars drawn" is checked). The deterministic spinner/toast behaviour is now covered by the Jest `useBackfillState` + `OhlcvView` specs; the e2e is a best-effort live smoke. A fake-fetcher seam (per the plan's original phase-4 description) would make it deterministic.
- **m4 (minor, `architect`/`dev`) — `data/backfill.py` imports `EventBus` from `api/events` (data→api direction smell).** Confined to the coordinator and documented in its docstring; there is **no actual import cycle** (`api/events` imports only pydantic — a self-contained leaf). Clean shape: relocate the `EventBus` + payload models to a neutral package (e.g. `market_analyser/events/`) that both layers depend on, or move the coordinator into the api layer. Tracked in the README open-followups; promote to a real plan only if the pattern recurs.
- **n5 (nit) — filename drift:** the plan named `tests/data/test_default_provider.py` for the phase-3 partial-failure cases; they landed (with full assertions) in `tests/data/test_default_provider_cache.py`. No action.
- **n6 (nit, folds into Plan 0017) — `mcp_app.py` grew ~145 lines** via the two new inline-closure tools, reinforcing Plan 0017's motivation to migrate the six original inline tools to the `register_*` pattern.
- **n7 (nit) — the coordinator's cache-only `coverage()` read runs synchronously on the event loop** inside `_run_backfill` (only the network fetch is offloaded to a thread). Fast local SQLite read at desktop scale; fine as-is.
