...
| 0015 | [0015-pnpm-dev-all](0015-pnpm-dev-all.md) | approved | Dev-tooling plan carved out from Plan 0007 smoke pain (sidecar in one terminal, `pnpm dev` in another, hand-edit `.mcp.json` for port+bearer every iteration). New root `package.json` with `pnpm dev:all` that uses `concurrently` to orchestrate a Python-sidecar wrapper (`scripts/dev/spawn-sidecar.mjs`), a `.mcp.json` writer (`scripts/dev/write-mcp-config.mjs` — reads `sidecar.lock` for port + `mcp-secret.json` for bearer, atomic-writes the repo-local `.mcp.json` on every lockfile change), and the existing `pnpm --filter @market-analyser/desktop dev` gated by `wait-on file:<data-dir>/sidecar.lock`. Default Ctrl+C tears the sidecar down (dev-only override of [ADR-0016](../adrs/0016-standalone-sidecar-mode.md)); `--keep-sidecar` opts back into ADR-0016's outlive-the-viewer semantics; running against a still-running sidecar reuses it (kill-only-what-you-spawned). Three phases, all `dev`. No production code changed; pure tooling. |
...
- **Numbering** is sequential and zero-padded to four digits. Next free number is **0016** (Plans 0013 — auto-backfill on cache miss — 0014 — interactive chart + agent-mode toggle — and 0015 — `pnpm dev:all` one-command dev startup — drafted 2026-05-22; the first two from Plan 0007's close-ceremony Followups, 0015 from the same session's smoke-pain discussion). ADR numbers are an independent sequence (see [`../adrs/`](../adrs/)) — next free ADR is **0022** (ADR-0021 proposed 2026-05-22 alongside Plan 0014; moves to accepted at Plan 0014 close after Phase 4 smoke; previous accepted ADRs: ADR-0020 accepted 2026-05-22 at Plan 0007 close, ADR-0019 / ADR-0018 proposed 2026-05-20, ADR-0017 accepted 2026-05-20). Architect runs `Glob docs/architecture/plans/*.md` and `Glob docs/architecture/adrs/*.md` before drafting to pick the next numbers, never trusting memory.
...
Don't guess. The execution sequence above is the source of truth as of 2026-05-22 (Plans 0001 + 0002 + 0003 + 0004 + 0005 + 0006 + 0007 closed; **Plan 0015 recommended next** as quality-of-life for every subsequent smoke; Plan 0008 next on the substantive thread, both gates satisfied; Tier 2 series 0009 → 0010 → 0011 → 0012 sequenced after 0008; Plan 0007 followup series 0013 → 0014 drafted 2026-05-22 and sequenced after Tier 2; ADR-0021 proposed alongside Plan 0014). If reality has drifted (the user names a plan not in that sequence, or a status disagrees with a recent commit), trust `git log` and the plan's own `Status:` line over this README — and surface the drift so the README gets refreshed.
