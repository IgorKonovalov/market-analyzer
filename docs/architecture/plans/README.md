...
- **0031** (approved, Yahoo absolute-range fetch) is a small `dev` data-layer fix: `_yahoo_fetch.py` + `yahoo.py` only. It **blocks 0030's completion** (0030's renderer is done but dead without it) and should be picked up promptly to unstick that work. It brushes the same `data/adapters/` surface as **0028** (adapter Protocol refactor) and **0022** (provider wiring), so don't run it in parallel with either — sequence on a quiet data-layer window. No collision with the analysis/UI batch.
