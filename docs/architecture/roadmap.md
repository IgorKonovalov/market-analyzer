...
- **DeFi data layer** — **largely realized**: Zerion-based wallet discovery ([Plan 0032](plans/done/0032-defi-wallet-discovery.md), done; [ADR-0034](adrs/0034-defi-portfolio-aggregator.md)/[ADR-0035](adrs/0035-defi-domain-placement.md)), deep LP detail ([Plan 0034](plans/0034-defi-deep-lp-detail.md), approved), and tx-replay P&L ([Plan 0035](plans/0035-defi-pnl-reconstruction.md)/[ADR-0036](adrs/0036-defi-pnl-reconstruction.md), approved). Consumed by the `defi-analyst` skill; positions under `positions/` (gitignored).
...
**Now committed (2026-06-05):** the forecasting subsystem is planned — [Plan 0036](plans/0036-forecasting-subsystem-foundation.md) (foundation) + [Plan 0037](plans/0037-forecast-ui-surface.md) (UI), implementing [ADR-0030](adrs/0030-forecasting-subsystem.md) (direction-as-probability, classical + gradient-boosted trees, walk-forward-beats-baseline gate) with [ADR-0040](adrs/0040-forecasting-model-artifacts.md) pinning the library (sklearn `HistGradientBoosting`, no extra native dep), the determinism mechanism, and model versioning/provenance. The first model targets **next-bar direction**; volatility and regime forecasting are later additions behind the same four invariants. This **resolves the "model versioning and determinism" cross-cutting ADR** flagged below (→ ADR-0040). The feeds-into-the-advisor link is realized by the advisory arc (below).
...
| ADR-needed                                          | Triggered by                            | Status (2026-06-05) |
|-----------------------------------------------------|-----------------------------------------|---------------------|
...
| Sidecar crash-supervision / scheduling              | First *scheduled* off-app workflow      | Open (standalone mode shipped — ADR-0016) |
| Notification transport                              | First user-facing alert                 | Open |
