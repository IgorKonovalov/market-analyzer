...
- **Live-quote path Binance-aware — RESOLVED 2026-06-13** (post-close direct follow-up). `DefaultMarketDataProvider.get_quote` is now membership-routed (mirrors `_ohlcv_route`), and `BinanceKlinesAdapter` implements `QuoteSource` via `GET /api/v3/ticker/24hr` (`lastPrice`→price, `priceChangePercent`→`change_pct` unscaled, `closeTime`→`as_of`; currency `"USDT"` only for `*USDT` pairs since the ticker payload carries no quote asset; the klines typed-error taxonomy reused). A Binance-only pair (e.g. `BTCUSDT`) now has a live price header. Surfaced by the phase-4 smoke; fixed forward — no new ADR (applies ADR-0052 membership + ADR-0007/0031 per-capability sources). `tests/data/test_binance_quote.py`.
