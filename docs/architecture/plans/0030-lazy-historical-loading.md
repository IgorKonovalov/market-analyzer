...
- **Done when:**
  ...
  - `live-chart.spec.ts` (Playwright) gains a case: open the app on a symbol/timeframe with more history available than the initial window, scroll/drag the chart to the left edge, and assert (a) the `ohlcv-history-loading` affordance appears, then clears, and (b) `window.__test_chart_render__` shows the candlestick series bar count **increased** after the load (older bars were prepended). Because this runs against live Yahoo, gate the "increased" assertion on the affordance having appeared (a warm full-history cache may already cover the view); document the best-effort nature in a comment, mirroring the 0013 phase-4 live-smoke caveat.
  - Manual smoke (in the phase commit message): with `pnpm dev` and a daily chart on a long-lived symbol, scroll left repeatedly — older bars stream in, the viewport stays anchored (no jump), the left-edge affordance blinks during each fetch, and paging stops cleanly at the start of available history with no error chip.
