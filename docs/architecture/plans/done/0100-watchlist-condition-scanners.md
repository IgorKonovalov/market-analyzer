...
> **Status:** done — closed 2026-07-15 (paired [ADR-0095](../adrs/0095-watchlist-scan-fanout-harness.md) accepted). Dev phases 1–4 landed directly in `main` (`3a5c784` harness + `squeeze_scan` walking skeleton; `042ddb6` `gainers_losers`; `4ed554f` `momentum_scan`; `c3c7050` `volume_breakout`+`smart_volume` refactored onto the shared `_scan_symbols` harness, behaviour-preserving). Clean Mode 4 — no blockers/majors. Verified at assertion level: the three new `_..._scan_response` suites pin real ranking orders (squeeze tightest-first, gainers biggest-move-first, momentum RSI-desc), the skip/dropped split (short-history/missing → `skipped`; filtered-out → dropped, not skipped), sign + divide-by-zero + boundary-inclusivity, and per-scanner truncation-invariance / no-lookahead (genuine field-by-field `as_of=t` vs pre-truncated comparisons, not stubs). Phase-4 refactor touched only the two tool modules — zero test edits — and both shipped volume suites pass unchanged; both tools now delegate to `_scan_symbols` with `MAX_SCAN_SYMBOLS` re-exported. ADR-0083 trio + ADR-0023 snapshot reused via `condition_snapshot` (no drift from `analyze_symbol`); conditions-only (ADR-0029) — no call-shaped field. 41 scanner + tool-count tests green; `EXPECTED_FULL_TOOLSET` +3; apiref regenerated per phase. Two documented non-blocking nits (gainers is a single-period move, not whole-window; flat 0.0% reads `direction:"up"`). Implemented in this working tree — no branch/worktree to merge or prune. Foundation of the upstream-parity batch — Plans 0101 + 0102 now unblocked. **Phase 5 (`human` live smoke) outstanding — deferrable like the other read-only smokes; does not gate close.**
> **Created:** 2026-07-13
