...
> **Status:** done — closed 2026-07-10. Four renderer-only commits on `main`, no branch, migration-free, no new ADR (reuses ADR-0061's grouped-legend interaction, consumes ADR-0045 candlestick identity): phase 1 rename `d94b77f` (`ui-builder`), phase 2 grouped legend + draw-on-select `9ef4ac5` (`ui-builder`), plus two phase-2 follow-ups — `9981638` (marker-hover pattern name + pattern-bar outline, wider legend, ghost-hover fix) and `6765bde` (a draggable, `localStorage`-persisted, accessible-splitter layers-panel width per the ADR-0039 renderer-owned-prefs convention — clamped [150, 600]px, ArrowLeft/Right keyboard resize; reviewed clean, its 3 tests assert separator semantics + keyboard-resize-and-persist + clamp-on-restore). Phase 3 (`human` live smoke) was verified in the 2026-07-12 consolidated live smoke (see [`consolidated-smoke.md`](../../consolidated-smoke.md)). Clean Mode 4 — no blockers, no majors, no minors; every phase-2 done-when read at the assertion level against the real `CandlestickChart`: a 5-marker / 2-group sweep draws only the most-recent group (`lastMarkers.length === 2`, never 5) so the 104-at-once wall is gone; exactly K legend rows each with its correct count (`layer-count` 3 / 2); toggling a group on adds exactly that group (2→5) and off removes it (5→2); the master toggle hides the whole layer (→0) without desyncing the per-group set (the doji checkbox stays checked, redraws to 2 on master-on); hovering a row grows that group's markers and clears on leave; the empty/error scan states still render (no candlestick rows, overlays + price-line intact). Phase 1 renamed the two look-alike buttons via the Plan 0069 `t()` catalogs (`chart.scanPatterns`→`chart.candlesticks` "Candlesticks", `chart.scanChartPatterns`→`chart.chartPatterns` "Chart patterns", en + ru) with the `scan-patterns-button`/`scan-chart-patterns-button` testids stable. The plan's intent held: the shared `LayersPanel` (Plan 0067's grouped-legend component) was **reused and generalized** — one legend model for both trendline groups and candlestick-marker groups — via a new pure guard-exempt `lib/candleGroups.ts` (group by (pattern, kind) with counts + first-seen order, most-recent-group default, layer-id round-trip, 14-pattern display names), mirroring `lib/trendlines.ts`; the god-component grew by wiring only, per the plan's risk-note mitigation. The coarse per-direction marker rows + the standalone "Pattern spans" row folded into the single master, as designed. Close gates: **625/625** renderer jest (63 suites), `typecheck` + `lint` clean. **Clears the last chart-file plan gating Plan 0072 phase 8** (0067 + 0068 already closed 2026-07-09).
> **Created:** 2026-07-09
