...
> **Status:** done (2026-07-02) — both phases landed on branch `plan-0039-advisor-ui-surface` (`9774ae0` dev: `recommendation.completed v1` event, exactly-once-on-success emission; `25e5ab1` ui-builder: Recommendations view), merged `--no-ff` to `main`. Clean Mode 4 review — no blockers, no majors; every named done-when read at the assertion level: exactly-one-envelope on success (directional and flat) with zero-on-failure pinned both before and after validation; the advisory label asserted prominent (first child of the panel) and the ADR-0025 boundary enforced as a spec (zero interactive elements + no action language); low-conviction quiet styling; Zod `safeParse` in the dispatcher with loud drop; TS↔pydantic parity pins `label` as the single-value `advisory` const. Open question resolved: levels stay in the panel; the agent pushes labeled `price_line` overlays through the existing chart channel when wanted. Post-merge gates green: full offline pytest, renderer jest 385/385 (46 suites), `mypy --strict`.
> **Created:** 2026-06-05
