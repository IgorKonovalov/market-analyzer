...
| 0032 | [0032-defi-wallet-discovery](0032-defi-wallet-discovery.md) | approved | **First plan of the DeFi wallet-analysis program.** Paste an EVM address → decoded positions across Ethereum/Base/Arbitrum/Optimism via Zerion ([ADR-0034](../adrs/0034-defi-portfolio-aggregator.md)). Stands up the `SecretsStore` ([ADR-0038](../adrs/0038-third-party-api-key-storage.md), accepts at close), `WalletPositionsSource`+`ZerionAdapter`, a `defi/` discovery service ([ADR-0035](../adrs/0035-defi-domain-placement.md)), and a `scan_wallet` MCP tool + SSE progress. Agent-driven (no paste-box UI — that's the later UI plan). Backend-only. 4 phases, all `dev`. |
| 0033 | [0033-ui-theme-and-visual-polish](0033-ui-theme-and-visual-polish.md) | approved | In-app **Light/Dark/System** theme control (Settings *Appearance* + header toggle) persisted in `localStorage`, applied pre-paint via a `data-theme` attribute ([ADR-0039](../adrs/0039-renderer-theming-localstorage.md), accepts at close) — no flash, no sidecar round-trip; plus a visual-polish pass (palette/elevation/type-scale/restyled chrome, both themes) and a theme-aware candlestick chart (candle/overlay/marker colors → tokens). Renderer-only. Edits `styles.css`/`App.tsx`/`SettingsView.tsx`/`CandlestickChart.tsx` — **collides with 0023 + 0026's chart phase; serialize.** 4 phases, all `ui-builder`. |
...
- **0032** (DeFi wallet discovery) is `approved` and pickup-ready. It opens a five-plan DeFi program (discovery → deep adapters → P&L → risk → UI); only discovery is authored so far. It adds a new `data/adapters/zerion.py` + a new top-level `defi/` package, so it shares no files with the active TradFi roster, but it is the **first authenticated data source** (Zerion key via the new `SecretsStore`) and the first plan to touch the new `defi/` tree. The remaining four DeFi plans are not yet written — they get authored after 0032 lands and proves the Zerion integration.
- **0033** (UI theme + visual polish) is `approved` and pickup-ready. Pure renderer/`ui-builder` work, no backend/migration. It edits the shared `styles.css` plus `App.tsx`/`SettingsView.tsx`/`CandlestickChart.tsx`, so it **overlaps two other renderer efforts and must be serialized**: land it **after Plan 0023** (News view, landed 2026-06-03 — also edits `App.tsx`/views), and **do not run its phase 4 (chart theming) in parallel with Plan 0026's `ui-builder` phase 3** (chart panel on the same `CandlestickChart`/`OhlcvView` surface). Because it rewrites the shared token file, it is inherently serial with any other in-flight `ui-builder` plan regardless of worktrees.
...
- **Numbering** is sequential, zero-padded to four digits, independent for plans and ADRs. **Next free plan: 0034. Next free ADR: 0040** (see [`../adrs/`](../adrs/)). Architect runs `Glob docs/architecture/plans/*.md` and `…/adrs/*.md` before drafting — never trusting memory.
