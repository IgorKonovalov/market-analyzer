...
> **Status:** draft
> **Created:** 2026-05-17
> **Owner skill(s):** `dev` / human (slices 1–2, 5), `strategy-author` (slice 4), `backtester` (slice 3)
> **Related ADRs:** [ADR-0004](../adrs/0004-strategy-interface.md), [ADR-0007](../adrs/0007-market-data-provider.md)
> **Depends on:** [Plan 0001](0001-bootstrap.md) slice 2 — `src/market_analyser/data/types.py` (which defines the canonical `Bar` model) must exist before slice 1 of this plan can land.
...
ADR-0004 captured *which* shape we chose; this plan captures *how we build it* and *who owns which slice*.
...
We implement ADR-0004 in five slices, smallest-valuable-thing first.

Slice 1 ships the contract module (`contracts/`) and a single trivial reference strategy (RSI) so that `strategy-author` has a concrete example to copy and `backtester` has a target to call. Slice 2 ships strategy discovery. Slice 3 ships the adapter that bridges new-shape signals to the vendored engine, so we don't have to rewrite metrics in slice 1. Slice 4 ports the remaining five vendored strategies. Slice 5 is the architect's review.
...
### Slice 1 — Contracts module + RSI reference strategy

- **Owner skill:** `dev` (writes contract definitions and the RSI module from the template defined below)
- **What:** Ship `src/market_analyser/contracts/{__init__.py,strategy.py}` with `Signal`, `SignalKind`, `BaseParams`, `StrategyMeta`, and `StrategyProtocol`. `Bar` is **imported from `market_analyser.data.types`** (created by Plan 0001 slice 2) — it is not redefined here, and there is no `contracts/market_data.py`. Ship `src/market_analyser/strategies/rsi.py` as the reference implementation that `strategy-author` will copy for the others.
- **Files touched:**
...
### Slice 3 — Signals-to-trades adapter

- **Owner skill:** backtester
- **What:** A `signals_to_trades(bars, signals)` function under `src/market_analyser/backtest/adapter.py` that consumes the new `Signal` event stream and produces the trade-dict shape the vendored `_apply_costs` / `_calc_metrics` / `_build_equity_curve` functions expect. This lets us reuse all the vendored metrics code unchanged in this slice and migrate it cleanly in a later plan.
- **Files touched:**
  - `src/market_analyser/backtest/__init__.py`
  - `src/market_analyser/backtest/adapter.py` (~60 lines)
  - `src/market_analyser/backtest/engine.py` — thin orchestrator: `run(strategy, bars, params, **costs) -> BacktestResult`. Imports the vendored metrics+costs helpers (vendored under `src/market_analyser/backtest/_vendored/` in a separate plan; for this slice, copy only `_apply_costs`, `_calc_metrics`, `_build_equity_curve`, `_buy_and_hold_return`).
...
### Slice 4 — Port the five remaining strategies

- **Owner skill:** strategy-author (driven by the template from slice 1)
...
### Slice 5 — CLI subcommand `strategies list`
...
These are the canonical contract definitions. They are **illustrative** in this plan — the authoritative version lives in `src/market_analyser/contracts/strategy.py` after slice 1. The `Bar` model is owned by `src/market_analyser/data/types.py` (Plan 0001 slice 2) and re-exported from `market_analyser.contracts` for ergonomic imports.
...
class SignalKind(str, Enum):
    ...
    EXIT_LONG  = "exit_long"
    # ENTER_SHORT / EXIT_SHORT reserved; not implemented in slice 1.
...
- **Risk: the adapter (slice 3) hides bugs.** If `signals_to_trades` is wrong, the golden test in slice 3 will compare two wrong numbers and pass. Mitigation: in slice 3, also write a unit test for the adapter alone (signals → trade dicts) using hand-rolled signal lists, not strategy output.
...
- **Open question: how does discovery handle a syntax error in `strategies/foo.py`?** Right now `discover()` will raise. Probably correct — a broken strategy file should fail loudly at startup, not be silently skipped. Confirm in slice 2 review.

## What this plan does NOT do

- **It does not build a backtest engine.** Slice 3 builds the *thinnest possible* engine that proves the contract end-to-end by reusing vendored metrics code. A real engine (with shorting, stops, walk-forward, parameter sweeps) is a separate plan that depends on this one.
- **It does not vendor the full `backtest_service.py` from tradingview-mcp.** Only the four helpers needed for slice 3 (`_apply_costs`, `_calc_metrics`, `_build_equity_curve`, `_buy_and_hold_return`). Full vendoring is a separate plan and a separate ADR if we choose to take it on at all.
...
- **It does not include short selling.** `SignalKind` reserves `ENTER_SHORT`/`EXIT_SHORT` but the slice-1 contract only ships `ENTER_LONG`/`EXIT_LONG`. Short selling is out of scope and tracked as a followup.
...
If any of these are wrong, correct in this section and re-derive affected slices.
