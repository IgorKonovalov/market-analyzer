...
- **A directional call requires every leg to agree**: the forecast's argmax direction, at least one live signal in the same direction with none opposing, and a positive walk-forward edge belonging to a strategy that actually voted. Anything less returns a **flat** recommendation whose rationale names each failed leg.
- **Conviction is derived, never invented**: `P(direction) × clamp(sharpe_mean / 1.0, 0, 1)` — monotone in the forecast probability and the backtested edge, zero when either shows nothing. A marginal edge *reads* as low conviction by construction. Do not round it up in prose.
