...
| 0008 | Refresh `docs/architecture/diagrams/claude-cli-driven-architecture.md` with a backtest-results lane | `architect` | The current map predates the `run_backtest` MCP tool + `BacktestView` + `RecentBacktestsView`. Add the lane so the system map reflects post-0008 reality. |
...
**Tier 2 (data breadth) ships as a series** — Plans 0009–0012 — is now recommended next. The series is anchored by [ADR-0019](../adrs/0019-external-http-adapter-resilience.md) (shared resilience module for every external HTTP adapter); each subsequent adapter inherits the module for free, so they sequence cheaply behind 0009. Sequencing rationale:
...
[Recommended next — Tier 2 data series; sequential, each inherits the resilience module from E1]
...
- **Numbering** is sequential and zero-padded to four digits. Next free number is **0016** (Plan 0008 closed 2026-05-23; the active roster now runs 0009 → 0010 → 0011 → 0012 → 0013 → 0014). ADR numbers are an independent sequence (see [`../adrs/`](../adrs/)) — next free ADR is **0022** (ADR-0021 proposed 2026-05-22 alongside Plan 0014; moves to accepted at Plan 0014 close after Phase 4 smoke; ADR-0018 accepted 2026-05-23 at Plan 0008 close; previous accepted ADRs: ADR-0020 accepted 2026-05-22 at Plan 0007 close, ADR-0019 proposed 2026-05-20, ADR-0017 accepted 2026-05-20). Architect runs `Glob docs/architecture/plans/*.md` and `Glob docs/architecture/adrs/*.md` before drafting to pick the next numbers, never trusting memory.
...
Don't guess. The execution sequence above is the source of truth as of 2026-05-23 (Plans 0001 + 0002 + 0003 + 0004 + 0005 + 0006 + 0007 + 0008 + 0015 closed; **Plan 0009 recommended next** as the Tier 2 anchor — resilience module + TradingView screener + Yahoo retrofit, four phases all `dev`; Tier 2 series 0009 → 0010 → 0011 → 0012 sequenced linearly because each inherits Plan 0009 phase 1; Plan 0007 followup series 0013 → 0014 drafted 2026-05-22 and sequenced after Tier 2; ADR-0018 accepted at Plan 0008 close 2026-05-23; ADR-0021 proposed alongside Plan 0014). If reality has drifted (the user names a plan not in that sequence, or a status disagrees with a recent commit), trust `git log` and the plan's own `Status:` line over this README — and surface the drift so the README gets refreshed.
