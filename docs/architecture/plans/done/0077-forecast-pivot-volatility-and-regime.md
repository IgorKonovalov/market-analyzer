...
> **Status:** done — closed 2026-07-11. Six code phases on `main`, no branch, migration-free: ph1 volatility+baseline `dd690ab`, ph2 regime+transition `5012d95`, ph3 tools+SSE+explainability `5813462`, ph5 advisor non-voting inputs+direction demotion `85366d3`, ph6 views `35a57c7`; ph4 cross-asset edge verdict artifact at `runs/analysis/2026-07-10-plan-0077-vol-regime-edge/` — an honest **null** result (no ML edge replicates across BTC+ETH; the deterministic EWMA/persistence baseline and the trailing rule-based regime are the useful shipped outputs; the one BTC h=21 regime beat is tier-robust but does not carry to ETH, so it fails the cross-asset overfit guard). [ADR-0070](../adrs/0070-non-directional-forecast-targets.md) + [ADR-0071](../adrs/0071-advisor-non-directional-inputs-and-direction-demotion.md) accepted at close. Clean Mode 4 — no blockers/majors/minors/nits; every named done-when read at the assertion level: forward-looking label with byte-identical past feature rows + baselines under a future-bar perturbation (no lookahead), honest `beats_baseline=False` on iid fixtures, `model_dump` determinism on both kinds, regime trend axis pinned `== _classify_trend`, exactly-once `*.completed` SSE with zero-on-failure, AST read-only source scan, and the ADR-0029 directional-agreement invariant intact (adverse vol/regime make a call smaller/wider-stopped/lower-conviction, never flip it or manufacture one; direction-demotion skill-margin boundary pinned both sides). Gates: 154 forecast/advisor/api pytest · 113 renderer jest · `apiref --check` clean · `mypy --strict` + ruff + tsc + eslint green. **Phase 7 (human live smoke + go/no-go on keeping the advisor vol/regime wiring active) DONE 2026-07-11 — verdict GO** (wiring stays active; it degrades correctly to deterministic baseline sizing + untrusted-regime neutrality on live ETH-USD 1d, where the ML tier has no edge). Full smoke record in Followups below.
> **Created:** 2026-07-10
...
- **Phase 7 live smoke — DONE 2026-07-11, verdict GO (advisor vol/regime wiring stays active).** Ran `forecast_volatility` / `forecast_regime` / `recommend` live on ETH-USD 1d through the running sidecar:
  - **Volatility (h=5):** honest no-edge (`beats_baseline=false`, model QLIKE 0.795 vs EWMA 0.591); deterministic baseline 0.0247 surfaced; v1 tier stated via `fallback_reason` (v2-deep 254/1653 rows < 500 floor). Re-run byte-identical → determinism confirmed live.
  - **Regime (h=21):** `beats_baseline=true` at a **noise-level** margin +0.0096 on Brier ~1.36 — and it **flipped sign** vs phase 4's ETH h=21 (−0.004) from a one-bar `range_end` shift. Reinforces (does not overturn) the phase-4 "no robust cross-asset edge" verdict; the flag is fold-fragile and must not be read as a durable edge.
  - **`recommend` (rsi):** direction-leg demotion working end-to-end (`direction_leg.gating=false`, skill margin −0.00076 < 0.02; the corroborated long is no longer vetoed by the no-edge forecast — the ADR-0071 fix on real data); volatility non-voting correctly falls back to the deterministic baseline for inverse-vol sizing + stop; regime non-voting untrusted at h=1 → neutral; conviction honestly low (0.149); directional-agreement invariant intact.
  - **Renderer check:** `ForecastView.tsx` gates the trusted band on `beats_baseline && predicted_vol != null`, so a real no-edge payload (which *does* carry a losing `predicted_vol`) renders the explicit no-edge state, not a misleading band. No finding.
  - **Rationale for GO:** the ML tier adds no edge on ETH, but the wiring *degrades correctly* — deterministic baseline sizing/stops + untrusted-regime neutrality — and can never flip or manufacture a direction. It provides value (baseline inverse-vol sizing) while refusing the noise-level ML signals.
- **Watch item (not a defect today):** the regime `beats_baseline` flag is sign-unstable at h=21. The advisor consumes regime only at h=1 (untrusted → neutral), so there is no live exposure — but any future change feeding h=21 regime into conviction would let a noise-level "trusted" beat leak in. Re-evaluate if/when regime is consumed at longer horizons.
