...
> **Status:** done — closed 2026-07-21. All three phases landed on `main` (no branch): `eb055c7` ph1+ph3 (`specs/` layer — `_template.md`, `README.md`, four seed specs: backtest-engine pilot + data-provider + advisory-boundary + mcp-tool-surface — plus the close-ceremony reconcile step added to architect SKILL.md) and `d56099e` ph2 (`market_analyser.specs` structural `--check` gate + CI wiring + `specs:check` script + `tests/apiref/test_specs_check.py`). **Clean Mode 4 — no blockers/majors/minors.** Every seed spec's invariants were read against the code they describe at close (the phase-3 self-authorship risk): the backtest-engine claims match `adapter.py:_ordering_key`/`signals_to_trades` + `_bars_hash.py` byte-for-byte, all 8 `MarketDataProvider` methods carry `as_of` (get_quote/search_symbols honestly noted as stubs), and `EXPECTED_FULL_TOOLSET` enumerates exactly 59 tools (event_calendar present, post-0113). Gate exits 0 on the four committed specs; 12/12 `test_specs_check.py` pass. `Reconciled-through:` bump skipped at this close — the specs are current as of Plan 0112 by construction. ADR-0106 accepted. Version bumped at close (feat + docs → minor).
> **Created:** 2026-07-21
