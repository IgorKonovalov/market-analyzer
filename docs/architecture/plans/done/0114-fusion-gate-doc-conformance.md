...
> **Status:** done — closed 2026-09-04. Four phases on `main`, no branch, run out of order (ph3 `architect` first, since `dev` refused the out-of-vocabulary ph4 tag and escalated — see the owner-tag note below): `e3bd672` ph3 (spec invariants + scenarios, `Reconciled-through:` → 0114) → `2311b1e` ph1 → `1ae5445` ph1 fix-forward (sixth drift site, found during ph2 mutation testing) → `4c08cf7` ph2 (three demoted-branch tests, mutation-verified) → `4235a7d` ph4 (advisor `SKILL.md` + `recommend-tool.md`). **Mode 4 verdict: no blockers.** Zero behavior change confirmed — the entire `src/` diff is confined to `fusion.py`'s module docstring and one function docstring; 122 advisor tests pass. Phase 2's tests are genuine, not stubs: `_demoted_conviction` asserts `direction_leg.gating is False` so it provably drives the demoted path, and the commit records the mutation that fails all three. **One finding, fixed at close:** ph4 identified two further drift sites (`fusion.py:25` and `:374`, both claiming *directional exactly when every check passed*) and correctly declined to cross into `dev`-owned code, flagging them instead; they are corrected in `1057528`. The runtime invariant was never at risk — `_replay_direction` has always filtered on `check.gating`. Spec already reconciled by ph3. No paired ADR (ADR-0071 stands as written). No version bump (docs/test only).
> **Created:** 2026-09-04
