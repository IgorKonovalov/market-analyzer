...
- **0033** (UI theme + visual polish) is `approved` and pickup-ready. Pure renderer/`ui-builder` work, no backend/migration. It edits the shared `styles.css` plus `App.tsx`/`SettingsView.tsx`/`CandlestickChart.tsx`, so it **overlaps two other renderer efforts and must be serialized**: land it **after Plan 0023** (News view, in flight on `plan-0023-news-view` — also edits `App.tsx`/views), and **do not run its phase 4 (chart theming) in parallel with Plan 0026's `ui-builder` phase 3** (chart panel on the same `CandlestickChart`/`OhlcvView` surface). Because it rewrites the shared token file, it is inherently serial with any other in-flight `ui-builder` plan regardless of worktrees.
