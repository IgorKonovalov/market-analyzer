...
| 0017 | [0017-consolidate-mcp-tool-registration](0017-consolidate-mcp-tool-registration.md) | draft | Maintainability refactor surfaced by the 2026-05-24 codebase health audit. Migrates the six original MCP tools (`get_ohlcv`, `write_annotation`, `list_annotations`, `show_chart`, `update_chart`, `highlight_pattern`) from inline `@server.tool` closures inside `create_mcp_components` to the per-module `register_*` pattern the two newest tools (`run_backtest`, `screener_query`) already use; shared `_require_*`/`_parse_overlays` helpers move to `mcp_tools/_validation.py`. Pure behavior-preserving — existing integration suites are the regression net; `mcp_app.py` drops from 326 to ~120 lines. No new ADR (implements an existing pattern). Two phases, all `dev`. Out-of-band; sequence before Plans 0010–0012 so the three new Tier 2 tools inherit one pattern. |
...
| 0016 | Each Tier-2 plan that ships a new agent-facing MCP tool adds one golden-path step to `tests/smoke/golden_path.py` at its close | `architect` (per Tier-2 close) | Plan 0016's smoke covers the agent-tool surface as of 2026-05-24. Plans 0010–0012 add new agent-facing tools (`news_for`, `sentiment_for_news`, `crypto_fear_greed`, `stocktwits_sentiment`); each close ceremony should add one golden-path line so the smoke doesn't rot (plan Risk: "smoke drifts as new tools ship"). One line per close; the closing `architect` session owns it. |
...
Don't guess. The execution sequence above is the source of truth as of 2026-05-24 (Plans 0001 + 0002 + 0003 + 0004 + 0005 + 0006 + 0007 + 0008 + 0009 + 0015 + 0016 closed; **Plan 0010 recommended next** — RSS news + per-headline VADER sentiment, three phases all `dev`, inheriting the resilience module Plan 0009 landed; remaining Tier 2 series 0010 → 0011 → 0012 sequenced linearly because each inherits Plan 0009 phase 1; Plan 0007 followup series 0013 → 0014 drafted 2026-05-22 and sequenced after Tier 2; ADR-0019 accepted at Plan 0009 close 2026-05-24; ADR-0018 accepted at Plan 0008 close 2026-05-23; ADR-0021 proposed alongside Plan 0014). Plan 0016 (golden-path smoke) **closed 2026-05-24** — out-of-band dev tooling on disjoint files from the Tier 2 series (`tests/smoke/`, root `package.json`, onboarding docs); it shipped `pnpm smoke` plus a renderer-only `DELETE /annotations/{id}` route for its cleanup step. Plan 0017 (MCP tool-registration refactor) is `draft` and also out-of-band, but with a soft ordering preference: land it **before** Plans 0010–0012 so the three new Tier 2 MCP tools inherit the consolidated `register_*` pattern instead of perpetuating the inline-closure split. It touches only `src/market_analyser/api/mcp_app.py` + `mcp_tools/`, so it conflicts with the Tier 2 series only at the `register_*` call site in `create_mcp_components` — cheap to rebase either way. If reality has drifted (the user names a plan not in that sequence, or a status disagrees with a recent commit), trust `git log` and the plan's own `Status:` line over this README — and surface the drift so the README gets refreshed.
