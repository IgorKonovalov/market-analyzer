...
> **Status:** done — closed 2026-07-09. Four `dev` phases on `main`, no branch, no migration: `ef770da` (wiring + introspection core, `test_mcp_tools.py` refactored onto the shared `apiref.wiring` helper — behaviour-preserving) → `c087d6e` (deterministic renderer) → `4c3dc03` (write/`--check` CLI + the committed `docs/reference/` tree) → `ed908a8` (CI drift gate + `pnpm gen:api-docs[:check]` + README/onboarding links). The phase-1 open question resolved without code: `events/TYPE_REGISTRY` already exists (last touched by Plan 0064), so introspection consumes it as-is — no new registry added. Clean Mode 4 — no blockers, no majors, no minors. Every done-when read at the assertion level: introspected tool-name set **==** the 42-name `EXPECTED_FULL_TOOLSET` (the same wiring the full-toolset test now shares — one source of truth for the conditional-registration wrinkle); the exact-block renderer test (params table with a required-positional `—` and an optional `` `None` ``, no-params line, output-schema field table vs. return-annotation fallback); **byte-identical across two independent wiring passes** (the property the gate stands on); `--check` **0** on the committed tree and **1** on a one-byte edit (both directions); the committed `mcp-tools.md` carries a full `forecast` entry (real description, `symbol`/`horizons`/… params, `MultiHorizonForecastResult` return). Close-time gates green: `apiref --check` exit 0, 27 apiref+toolset specs, `mypy --strict` + ruff clean over `apiref/`. Determinism verified safe on Windows checkout (`git check-attr eol docs/reference/*.md` → `lf`). Generated surface: 42 MCP tools, 25 REST routes, 20 SSE events. **ADR-0064 accepted at close.** Phase-4 CI-fails-on-drift is exercised by the in-process `test_check_detects_one_character_drift`; a live red-CI demonstration on a throwaway branch is the user's optional confirmation, not a code gate.
> **Created:** 2026-07-09
> **Owner skill(s):** dev
