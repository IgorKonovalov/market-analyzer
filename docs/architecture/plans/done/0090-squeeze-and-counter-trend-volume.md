...
> **Status:** done — closed 2026-07-13 ([ADR-0083](../adrs/0083-squeeze-and-counter-trend-volume-semantics.md) accepted at close). All four `dev` phases on `main`, no branch, migration-free, no new dep: `63cf241` ph1 (`keltner()` + `bollinger_bandwidth()` primitives — new `KeltnerValue`, both trailing/length-aligned/`None`-prefixed, div-by-zero guard) → `f9bdc9a` ph2 (snapshot `bb_width` / `bb_width_pct90` / `squeeze_on` — TTM BB-inside-Keltner on the latest bar, `1.0`/`0.0` like `supertrend_direction`, honest `None` on too-few bars; `atr_pct90` kept as a volatility fact, no longer the squeeze proxy; frozen 30-key indicators guard added) → `adb5a74` ph3 (`counter_trend_volume()` + frozen `extra=forbid` `CounterTrendBar`/`CounterTrendVolume` models — per-bar direction (close-vs-open, the pinned basis) + trailing relative volume + counter-trend flag anchored to the supplied snapshot `trend`; `sideways` → `anchored_to_sideways`, share `None`, no bar flagged) → `12f49f7` ph4 (`counter_trend_volume` MCP tool mirroring the `volume_confirmation` `{result, partial_reason, scanned_at}` envelope, anchored to `condition_snapshot(...).trend`, honest `no_bars` on empty cache, trailing `as_of` replay; `EXPECTED_FULL_TOOLSET` 50→51, apiref regenerated). Clean Mode 4 — **no blockers/majors/minors/nits**. Every named test read at the assertion level, all genuine (no stubs, no missing specs): ph1 pins band-width + Keltner against independent mean/pstdev + EMA/ATR references within `1e-9`, truncation-invariance + `None`-prefix length via the generic `_INDICATORS` suite, zero-middle → `None`, bad-param `ValueError`s; ph2 pins the exact 30-key `indicators` set (drop/stray fails loudly), squeeze `1.0` inside-channel + `0.0` wide-band (both re-verified against `bollinger()`/`keltner()` directly), honest `None` short-series, independent `bb_width_pct90` percentile; ph3 pins the hand-computed UP share `0.75` + its DOWN mirror `0.25`, the sideways-undefined case, truncation-invariance (overlap-window entries identical across truncations), `extra=forbid`, determinism, and the BTC-probe divergence shape; ph4 pins anchor `== analyze_symbol`'s trend on identical bars, `no_bars`, and `as_of` trailing-replay matching a direct computation. Gates re-verified at close: **2475 Python passed / 7 known Windows POSIX skips**, `mypy --strict` + `ruff` clean on all touched source, `apiref --check` exit 0. Implemented directly in this working tree — no branch/worktree to merge or prune. **Phase 5 (`human` live smoke) run by architect at close via MCP and PASS:** `analyze_symbol ETH-USD 1d` → `bb_width_pct90`=78.9 vs `atr_pct90`=3.3 (the contradiction now visible/resolvable in one snapshot), `squeeze_on`=0; `BTC-USD 1d` → `bb_width_pct90`=43.3 vs `atr_pct90`=4.4, `squeeze_on`=0; `counter_trend_volume BTC-USD 1d` (trend=down) surfaced the per-bar split (heavy counter-trend rallies vs thin bars, share 0.53) and `ETH-USD 1d` (trend=sideways) returned the explicit `anchored_to_sideways`/share-`null`/no-flag result; nothing in either output reads as a buy/sell call. Numbers drifted from the plan's authoring-day snapshot (ETH band-width 73rd→79th; BTC 36th→43rd; BTC trend flipped up→down) — that is the market moving; the qualitative claims and the tool's correct live-trend anchoring hold, which the plan explicitly sanctions. Followups (unchanged): Keltner overlay + squeeze marker render (`ui-builder`); squeeze-trajectory field; advisor consumption of squeeze / counter-trend divergence (ADR-0029 scope); squeeze-breakout strategy / `create_watch` alert.
> **Created:** 2026-07-12
