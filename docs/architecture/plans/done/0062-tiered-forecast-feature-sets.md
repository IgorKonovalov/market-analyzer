...
> **Status:** done — closed 2026-07-07. Phases 1–2 `dev` on `main` (`126a87c` v2-deep feature set → `c0050cf` ladder selection), no branch, migration-free; phase 3 `backtester` honesty artifact `runs/analysis/2026-07-06-plan-0062-v1-vs-v2deep/` (with a beyond-plan `v1-deep-sample` ablation that turned the stated window confound into a measurement); phase 4 verdict user-stated 2026-07-07 after a pre-close robustness sweep (`runs/analysis/2026-07-07-plan-0062-h21-robustness/`): **promising, not proven** — v2-deep is the first exogenous tier ever to train and passes the walk-forward gate at all three horizons under the shipping config, but the h=21 clear edge (+0.0728) is fold-layout-sensitive (no_edge / clear / marginal across n_splits 4/5/6 — the layout decides the scored window and its baseline), while the feature contribution over identical-sample v1 is steadier (+2 to +3.3 pts in 2 of 3 layouts, never materially negative; seeds are a non-axis — the fit is deterministic). No new-sources plan now; re-run the comparison + sweep when accrual widens the deep window / v2-full crosses the floor (standing follow-up). Clean Mode 4 — no blockers, no majors; every done-when read at the assertion level; close-time gates: 38/38 plan specs, full suite 1908 passed / 7 Win-skips (lone red = the standing calendar-blind Yahoo `network` smoke, a 0041-close follow-up), `mypy --strict` + ruff green. ADR-0057 accepted at close.
> **Created:** 2026-07-06
