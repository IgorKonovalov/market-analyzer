...
| [0059](0059-trendline-event-channel-and-recompute.md) | Trendlines on a dedicated `chart.trendlines v1` event, recomputed on chart load/range change (never persisted) | proposed (accepts at Plan 0064's close) | partially supersedes 0049 (trendline wire shape: off `chart.show`/`chart.update`); follows 0045's dedicated-channel precedent; rides 0017's stream | 0064 |
...
- **Numbering** is sequential, zero-padded to four digits, never reused. **Next free ADR number is 0061** (0060 — glossary tooltip interaction posture, proposed 2026-07-08, paired with Plan 0065 — is the highest drafted; the roster above is the full inventory, and per-ADR drafting history lives in each file's `Date:` header + `git log`). Still `proposed` and awaiting their paired plan's close: 0037 (DeFi risk — Plan 0042), 0041 (Polymarket — Plan 0040), 0043/0044 (execution — Plan 0044), 0057 (feature-set tiers — Plan 0062), 0058 (explainability — Plan 0063), 0059 (trendline channel — Plan 0064), 0060 (glossary tooltips — Plan 0065), plus the exploratory 0025 (accepts at Plan 0046's close). The architect runs `Glob docs/architecture/adrs/*.md` before drafting to pick the next number, never trusting memory. ADR numbers and plan numbers are independent sequences.
