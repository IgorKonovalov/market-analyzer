...
> **Status:** done (closed 2026-07-02) — Pillar 2's advisor layer: the one sanctioned ADR-0029 crossing, landed as designed. Phase 1 (`advisor/` package: frozen `Recommendation`/`RecommendationBasis` + pure `fuse()`) `47e7ac9`; phase 2 (`recommend` MCP tool) `339885e`; two fix-forward commits: `97099a5` (hoisted `ForecastResult` into `forecast/result.py` so the advisor consumes a domain surface, not a tool module) and `6455786` (fuse() validates symbol/timeframe/as-of alignment across all four legs + requires the walk-forward edge to belong to an agreeing strategy). Clean Mode 4 close — no blockers, no majors; every done-when read at the assertion level: basis-free ⇒ `ValidationError` (plus directional-without-backtest/forecast-basis/rationale/levels, and flat-with-levels/nonzero-conviction, all pinned); conviction = P(direction) × clamp(sharpe_mean/1.0, 0, 1) pinned as the exact product and monotone in both inputs, saturating at full credit; determinism via dump-equality; the import-lint walks the package AST (outputs-only imports); the no-key/no-order/no-network source-level test scans advisor + tool for forbidden tokens and imports; honest-flat blockers named per failed leg; registration asserted against the live toolset. One honestly-documented deviation: registration lives in `mcp_app.py` (the Plan 0017 consolidation), not the plan's stale `mcp_tools/__init__.py` file list. 1591 passed / 7 Win-skips, `mypy --strict` (341 files) + ruff clean. **ADR-0029 accepted at close.** Migration-free; implemented directly on `main` (no branch). **Unblocks 0039** (advisor UI). Companion work (create `advisor` skill; reconcile `defi-analyst` rebalance charter) still open — carried in the plans-README follow-ups.
> **Created:** 2026-06-05
> **Owner skill(s):** dev
