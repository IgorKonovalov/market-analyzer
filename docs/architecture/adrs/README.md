...
| [0057](0057-forecast-feature-set-tiers.md) | Forecast feature sets tier by historical depth; the richest eligible tier trains (`v2-full → v2-deep → v1`, 500-row floor, stated skip chain) | proposed (accepts at Plan 0062's close) | tiers 0054's set (lag-1 + no-imputation hold within every tier); responds to 0051's per-series depth asymmetry; stops waiting on 0056's warm-up; preserves 0030's invariants; ids flow into 0040 | 0062 |
| [0058](0058-forecast-recommendation-explainability.md) | Forecast & recommendation explainability: OOS permutation importances + fusion trace, summary inline + full artifact under `runs_dir` | proposed (accepts at Plan 0063's close) | extends 0030 (how-sure → why) + 0040 (provenance); makes 0029's basis replayable; delivery split per 0046; ranks whichever 0057 tier trained | 0063 |
...
- **Numbering** is sequential, zero-padded to four digits, never reused. **Next free ADR number is 0059** (0058 — forecast & recommendation explainability, proposed 2026-07-06, paired with Plan 0063 — is the highest drafted; the roster above is the full inventory, and per-ADR drafting history lives in each file's `Date:` header + `git log`). Still `proposed` and awaiting their paired plan's close: 0037 (DeFi risk — Plan 0042), 0041 (Polymarket — Plan 0040), 0043/0044 (execution — Plan 0044), 0057 (feature-set tiers — Plan 0062), 0058 (explainability — Plan 0063), plus the exploratory 0025 (accepts at Plan 0046's close). The architect runs `Glob docs/architecture/adrs/*.md` before drafting to pick the next number, never trusting memory. ADR numbers and plan numbers are independent sequences.
